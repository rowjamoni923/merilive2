...
BEGIN
  ...
  v_left_count := CASE WHEN coalesce(v_auto ->> 'left_face_count', '') ~ '^-?\d+$' THEN (v_auto ->> 'left_face_count')::int ELSE 0 END;
  v_right_count := CASE WHEN coalesce(v_auto ->> 'right_face_count', '') ~ '^-?\d+$' THEN (v_auto ->> 'right_face_count')::int ELSE 0 END;
  ...
  v_front_yaw := CASE WHEN coalesce(v_auto ->> 'front_pose_yaw', '') ~ '^-?\d+(\.\d+)?$' THEN (v_auto ->> 'front_pose_yaw')::numeric ELSE NULL END;
  v_left_yaw := CASE WHEN coalesce(v_auto ->> 'left_pose_yaw', '') ~ '^-?\d+(\.\d+)?$' THEN (v_auto ->> 'left_pose_yaw')::numeric ELSE NULL END;
  v_right_yaw := CASE WHEN coalesce(v_auto ->> 'right_pose_yaw', '') ~ '^-?\d+(\.\d+)?$' THEN (v_auto ->> 'right_pose_yaw')::numeric ELSE NULL END;
  ...
  IF v_left_count = 1 AND v_fl > 0 AND v_fl < 50 THEN
    RETURN jsonb_build_object('success', false, 'reason', 'low_similarity_left', 'compare_front_left', v_fl, 'manual_review_requested', true, 'expected_gender', v_expected_gender);
  END IF;
  IF v_right_count = 1 AND v_fr > 0 AND v_fr < 50 THEN
    RETURN jsonb_build_object('success', false, 'reason', 'low_similarity_right', 'compare_front_right', v_fr, 'manual_review_requested', true, 'expected_gender', v_expected_gender);
  END IF;
  ...
  IF v_front_yaw IS NOT NULL AND v_left_yaw IS NOT NULL AND v_right_yaw IS NOT NULL THEN
    ...
    IF v_left_delta < 5 AND v_right_delta < 5 AND v_lr_delta < 10 THEN
  ...
  SET status = 'approved',
      ...
      admin_notes = CASE
        WHEN trim(coalesce(admin_notes, '')) = '' THEN
          '[auto] service_auto_finalize_face_verification: account type matched live face and required AI checks passed.'
        ELSE
          trim(admin_notes) || E'\n[auto] service_auto_finalize_face_verification: account type matched live face and required AI checks passed.'
      END,
      updated_at = now()
...
BEGIN
  ...
  v_detected := coalesce(nullif(v_final, ''), nullif(v_raw, ''));
  v_gender_conf := CASE
