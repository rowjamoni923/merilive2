...
BEGIN
  ...
  SELECT
    ...
    -- in-call badge only meaningful for verified female hosts
    CASE WHEN e.verified_female_host
         THEN (COALESCE(e.is_in_call, false) OR e.has_active_call)
         ELSE false END,
    COALESCE(e.is_host, false),
    e.gender,
    -- Hide call rate only when host is currently in another call;
    -- still surface rate when offline so user can attempt a call (server
    -- will reject with friendly "Host Offline" via start_private_call — Pkg336).
    CASE WHEN e.verified_female_host
  ...
  WHERE
    (
      -- All approved, face-verified female hosts surface on popular/new/following
    ...
    OR
    (
      -- Male hosts still only shown on live (if live) or following (if followed)
