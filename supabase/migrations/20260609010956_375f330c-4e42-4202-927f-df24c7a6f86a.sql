...
BEGIN
  ...
  -- PR-2.2: entry fee (coins), 0-100000, clamp negatives to 0.
  v_fee := greatest(0, coalesce(p_entry_fee, 0));
  IF v_fee > 100000 THEN
    RAISE EXCEPTION 'Entry fee too high (max 100000 coins)';
  END IF;
  ...
  INSERT INTO public.party_rooms (
    ...
    password_hash, is_locked, total_seats, max_participants, is_active, entry_fee
  )
  VALUES (
    v_uid, v_trim_name, v_type,
    nullif(trim(coalesce(p_game_mode, '')), ''),
    v_hash, v_locked, v_total, v_max, true, v_fee
  )
