...
BEGIN
  ...
  WHERE
    CASE v_tab
      ...
      -- Popular: verified female hosts AS BEFORE, PLUS any host currently live
      -- streaming (so no live host is ever missing from the default feed).
      -- Chamet/Bigo/Poppo parity: an active live tile ALWAYS surfaces on home,
      -- even if the streamer is not a verified female host.
      WHEN 'popular' THEN e.verified_female_host OR (e.is_live_now AND COALESCE(e.is_host, false))
      WHEN 'new' THEN
  ...
  ORDER BY
    ...
    CASE WHEN e.has_active_call THEN 1 ELSE 0 END,
