...
BEGIN
  ...
  -- Update profile: deduct coins + set VIP status + equip assets
  UPDATE profiles
  SET 
    coins = v_new_coins,
    ...
    equipped_frame_id = COALESCE((p_equip_updates->>'equipped_frame_id'), equipped_frame_id),
    equipped_entrance_id = COALESCE((p_equip_updates->>'equipped_entrance_id'), equipped_entrance_id),
    equipped_bubble_id = COALESCE((p_equip_updates->>'equipped_bubble_id'), equipped_bubble_id),
    previous_frame_id = COALESCE((p_equip_updates->>'previous_frame_id'), previous_frame_id),
    previous_entrance_id = COALESCE((p_equip_updates->>'previous_entrance_id'), previous_entrance_id),
    previous_bubble_id = COALESCE((p_equip_updates->>'previous_bubble_id'), previous_bubble_id)
  WHERE id = p_user_id;
