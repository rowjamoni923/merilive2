...
BEGIN
  ...
  -- Pkg auto-approve hardening: side-angle CompareFaces is unreliable because
  -- users intentionally turn left/right, so do not block safe approval on those
  -- audit scores. Require exactly one face, valid gender, strong gender + face
  -- confidence, adult estimate, and no strong occlusion flag.
  IF v_gender_conf < 80 OR v_face_conf < 80 THEN
...
REVOKE ALL ON FUNCTION public.service_auto_finalize_face_verification(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.service_auto_finalize_face_verification(uuid) FROM anon, authenticated;
