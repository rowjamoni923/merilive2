...
-- Step 1: Sync trigger function
CREATE OR REPLACE FUNCTION public.sync_daily_login_legacy_columns()
RETURNS trigger
LANGUAGE plpgsql
SET search_path TO 'public'
AS $$
BEGIN
  NEW.reward_amount := COALESCE(NEW.reward_coins, 0) + COALESCE(NEW.reward_diamonds, 0);
  NEW.reward_type := CASE
    WHEN COALESCE(NEW.reward_diamonds, 0) > COALESCE(NEW.reward_coins, 0) THEN 'diamonds'
    ELSE 'coins'
  END;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_sync_daily_login_legacy ON public.daily_login_rewards_config;
CREATE TRIGGER trg_sync_daily_login_legacy
BEFORE INSERT OR UPDATE ON public.daily_login_rewards_config
FOR EACH ROW
EXECUTE FUNCTION public.sync_daily_login_legacy_columns();

-- Step 2: Backfill existing rows
UPDATE public.daily_login_rewards_config
SET reward_coins = reward_coins;  -- triggers the sync above
