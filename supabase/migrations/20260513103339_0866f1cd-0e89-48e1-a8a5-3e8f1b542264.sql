...
BEGIN
  IF NEW.equipped_frame_id IS NOT NULL THEN
    IF NOT EXISTS (SELECT 1 FROM public.avatar_frames af WHERE af.id = NEW.equipped_frame_id)
       AND NOT EXISTS (SELECT 1 FROM public.shop_items si WHERE si.id = NEW.equipped_frame_id AND si.category IN ('frame', 'portrait_frame')) THEN
      RAISE EXCEPTION 'Invalid equipped_frame_id: avatar frame slot only accepts avatar_frames or frame/portrait_frame shop_items';
    END IF;
  END IF;
...
DROP TRIGGER IF EXISTS trg_validate_profile_equipped_assets ON public.profiles;
CREATE TRIGGER trg_validate_profile_equipped_assets
BEFORE INSERT OR UPDATE OF equipped_frame_id, equipped_entrance_id, equipped_entry_banner_id, equipped_entry_name_bar_id, equipped_vehicle_id
ON public.profiles
FOR EACH ROW
EXECUTE FUNCTION public.validate_profile_equipped_assets();