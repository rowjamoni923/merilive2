-- Face Verification admin RPC parity fix.
-- Source of truth:
-- 1) Queue status for normal submissions.
-- 2) profiles.is_face_verified=true wins as an approved Profile Audit row for legacy/profile-only verified users.
-- 3) retry_required is a separate user_retry bucket, never Pending.
...
BEGIN
  ...
  WITH latest AS (
    ...
    SELECT
      CASE
        WHEN coalesce(p.is_face_verified, false) THEN 'approved'
        WHEN retry_required THEN 'user_retry'
        ELSE public.face_verification_status_bucket(status)
      END AS status_bucket,
      CASE
        WHEN coalesce(p.is_face_verified, false) THEN false
      ...
      CASE
        WHEN coalesce(p.is_face_verified, false) AND public.face_verification_status_bucket(status) <> 'approved' THEN 'profile_verified'
        WHEN retry_required THEN 'needs_retry'
        ELSE lower(trim(coalesce(status, '')))
      END AS raw_status,
      CASE
        WHEN coalesce(p.is_face_verified, false) AND public.face_verification_status_bucket(status) <> 'approved' THEN true
        ELSE false
...
BEGIN
  ...
  WITH latest AS (
    ...
    SELECT *,
      ...
      CASE
        WHEN coalesce(is_face_verified, false) THEN false
        WHEN retry_required THEN false
        ELSE public.face_verification_is_auto_reviewed(status, admin_notes, verification_method)
...
REVOKE ALL ON FUNCTION public.admin_face_verification_stats(text) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.admin_face_verification_stats() FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.admin_list_face_verification_paginated(text,text,integer,integer) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.admin_face_verification_stats(text) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.admin_face_verification_stats() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.admin_list_face_verification_paginated(text,text,integer,integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.face_verification_is_retry_required(text,text,jsonb,text,text,text,text,text,text[]) TO authenticated, service_role;