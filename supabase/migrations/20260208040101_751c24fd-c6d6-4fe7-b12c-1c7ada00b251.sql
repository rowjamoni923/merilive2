-- Fix distribute_period_rewards: use gift_transactions instead of gift_transaction_logs, 
-- filter hosts by is_host=true, use beans instead of beans_balance
...
BEGIN
  ...
  -- Process host_earnings category - ONLY hosts (is_host = true)
  ...
  -- Process top_gifters category (using gift_transactions - actual data)
    ...
    FOR v_entry IN (
      ...
      IF v_reward IS NOT NULL THEN
        ...
        INSERT INTO notifications (user_id, type, title, message, data, is_read) VALUES (
          ...
          'Congratulations! You ranked #' || v_rank || ' in ' || 
          CASE p_period_type WHEN 'daily' THEN 'Daily' WHEN 'weekly' THEN 'Weekly' ELSE 'Monthly' END ||
          ' Top Gifter Rankings! You gifted ' || v_entry.stat_value || ' coins total. Rewards: ' ||
          CASE WHEN COALESCE(v_reward.reward_diamonds, 0) > 0 THEN v_reward.reward_diamonds || ' Diamonds ' ELSE '' END ||
          CASE WHEN COALESCE(v_reward.reward_beans, 0) > 0 THEN v_reward.reward_beans || ' Beans ' ELSE '' END ||
          CASE WHEN COALESCE(v_reward.reward_coins, 0) > 0 THEN v_reward.reward_coins || ' Coins' ELSE '' END,
          jsonb_build_object('category', p_category, 'period_type', p_period_type, 'rank', v_rank,
            'reward_beans', COALESCE(v_reward.reward_beans, 0), 'reward_diamonds', COALESCE(v_reward.reward_diamonds, 0),
            'reward_coins', COALESCE(v_reward.reward_coins, 0), 'total_gifted', v_entry.stat_value), false);

        v_count := v_count + 1;
      END IF;
    END LOOP;

  -- Process agency_performance category - AGENCIES GET BEANS
  ELSIF p_category = 'agency_performance' THEN
    v_rank := 0;
    FOR v_entry IN (
      WITH host_earnings AS (
        SELECT ah.agency_id, ah.host_id,
          COALESCE((SELECT SUM(FLOOR(coin_amount * 0.6)) FROM gift_transactions WHERE receiver_id = ah.host_id AND created_at >= v_start_date AND created_at < v_end_date), 0) +
      ...
      IF v_reward IS NOT NULL THEN
        ...
        IF v_entry.owner_id IS NOT NULL THEN
          INSERT INTO notifications (user_id, type, title, message, data, is_read) VALUES (
            ...
            CASE p_period_type WHEN 'daily' THEN 'Daily' WHEN 'weekly' THEN 'Weekly' ELSE 'Monthly' END ||
            ' Agency Rankings! ' || COALESCE(v_reward.reward_beans, 0) || ' Beans credited to your agency.',
            jsonb_build_object('category', p_category, 'period_type', p_period_type, 'rank', v_rank,
