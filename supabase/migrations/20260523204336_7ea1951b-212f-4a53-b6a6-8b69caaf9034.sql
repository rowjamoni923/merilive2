-- Relax auto-approve thresholds for face-verification so most clean submissions
-- pass automatically. Only declared-gender mismatch (handled by edge fn) leads
-- to auto-reject; every other "not sure" case goes to admin manual review.
...
BEGIN
  ...
  END IF;
  -- side-face count: if missing entirely treat as manual; if 0/many → manual
  ...
  -- ▼ RELAXED thresholds (target: ~90% auto-approve rate).
  -- Gender 86→70, face_conf 80→70.
  ...
  -- Front↔side similarity: 72→55 (Rekognition often returns 60-70 for natural
  -- side angles; 55 keeps real users while still catching different people).
  ...
  -- Pose deltas: 7/12 → 3/5. If yaw data missing, skip (don't block).
  ...
  -- Occlusion 88→95 (only block on very obvious mask/sunglasses).
  IF v_occ > 95 THEN
