...
-- Fix game_winners: use 'amount' column instead of 'win_amount'
...
BEGIN
  ...
  IF p_category = 'host_earnings' THEN
    FOR v_entry IN (
      ...
      IF v_reward IS NOT NULL THEN
        ...
        IF v_reward.reward_beans > 0 THEN PERFORM add_beans_to_user(v_entry.user_id, v_reward.reward_beans); END IF;
        IF v_reward.reward_diamonds > 0 THEN PERFORM add_diamonds_to_user(v_entry.user_id, v_reward.reward_diamonds); END IF;
        IF v_reward.reward_coins > 0 THEN PERFORM add_coins_to_user(v_entry.user_id, v_reward.reward_coins); END IF;
  ...
  -- game_winners (uses 'amount' column, transaction_type='win')
  ...
  IF p_category = 'agency_performance' THEN
    ...
    FOR v_entry IN (
      ...
      WHERE gt.created_at >= v_start_date AND gt.created_at < v_end_date AND a.is_active = true
      GROUP BY a.id HAVING SUM(FLOOR(gt.coin_amount * 0.6)) > 0
      ORDER BY stat_value DESC LIMIT 50
    ) LOOP
      v_rank := v_rank + 1;
      SELECT * INTO v_reward FROM leaderboard_reward_config WHERE category = p_category AND period_type = p_period_type AND is_active = true AND v_rank >= rank_from AND v_rank <= rank_to LIMIT 1;
