...
BEGIN
  ...
  -- Pkg381 fix: app_settings uses (setting_key, setting_value), not (key, value).
  ...
  -- Fallback defaults so the flow never silently breaks if admin hasn't set values yet.
  IF v_host_beans IS NULL OR v_host_beans <= 0 THEN v_host_beans := 10000; END IF;
  IF v_user_diamonds IS NULL OR v_user_diamonds <= 0 THEN v_user_diamonds := 5000; END IF;
