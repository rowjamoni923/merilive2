...
BEGIN
  ...
  SELECT (
    ...
    AND COALESCE(is_face_verified, false)
  ...
  INSERT INTO public.chat_moderation_logs (
    ...
    CASE
      WHEN v_is_verified_host THEN 'beans_deducted_2000'
      ELSE 'warning_only'
    END,
    true,
    v_notes
...
CREATE OR REPLACE FUNCTION public.reset_host_weekly_policy_after_withdrawal(
  p_host_id uuid
)
RETURNS jsonb
LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION public.trg_reset_host_policy_on_withdrawal_complete()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF (TG_OP = 'INSERT' AND lower(COALESCE(NEW.status, '')) IN ('approved', 'completed'))
     OR (TG_OP = 'UPDATE'
         AND lower(COALESCE(NEW.status, '')) IN ('approved', 'completed')
         AND lower(COALESCE(OLD.status, '')) NOT IN ('approved', 'completed')) THEN
    PERFORM public.reset_host_weekly_policy_after_withdrawal(NEW.host_id);
  END IF;
...
DROP TRIGGER IF EXISTS trg_reset_host_policy_on_withdrawal_complete ON public.helper_withdrawal_requests;
CREATE TRIGGER trg_reset_host_policy_on_withdrawal_complete
AFTER INSERT OR UPDATE OF status
ON public.helper_withdrawal_requests
FOR EACH ROW
EXECUTE FUNCTION public.trg_reset_host_policy_on_withdrawal_complete();

GRANT EXECUTE ON FUNCTION public.reset_host_weekly_policy_after_withdrawal(uuid) TO authenticated, service_role;