-- Fix: hourly live bonus was only crediting on program_day=1 because settings
-- table only has day_number=1 rows. New logic: day_number=1 is the daily
-- template; progress rows still keyed by program_day so each calendar day
-- resets independently and remains hard-capped at 5 hours × 60 minutes.
...
BEGIN
  ...
  _program_day := GREATEST(1, (EXTRACT(EPOCH FROM (now() - _host_created_at)) / 86400)::int + 1);

  -- Eligibility window: read from settings (eligible_days / eligible_program_days on day_number=1 row).
  SELECT COALESCE(MAX(GREATEST(COALESCE(eligible_days, 0), COALESCE(eligible_program_days, 0))), 0)
    INTO _eligible_days
  FROM public.new_host_live_bonus_settings
  WHERE is_active = true AND day_number = 1;
  IF _eligible_days = 0 THEN _eligible_days := 365; END IF;
  IF _program_day > _eligible_days THEN RETURN NEW; END IF;
  ...
  -- Template = day_number=1 rows (the 5 hourly slots × 10K beans).
  FOR _hour_row IN
...
-- 30-day backfill with corrected per-day logic.
DO $$
DECLARE
  s RECORD;
  _host_created_at timestamptz;
  ...
  _reset_date date;
BEGIN
  SELECT COALESCE(MAX(GREATEST(COALESCE(eligible_days, 0), COALESCE(eligible_program_days, 0))), 365)
    INTO _eligible_days
  FROM public.new_host_live_bonus_settings
  WHERE is_active = true AND day_number = 1;
  ...
  FOR s IN
    ...
    WHERE ls.ended_at IS NOT NULL AND ls.ended_at > now() - interval '30 days'
    ORDER BY ls.ended_at ASC
  LOOP
    SELECT created_at INTO _host_created_at FROM public.profiles WHERE id = s.host_id;
    _program_day := GREATEST(1, (EXTRACT(EPOCH FROM (s.ended_at - _host_created_at)) / 86400)::int + 1);
    IF _program_day > _eligible_days THEN CONTINUE; END IF;
