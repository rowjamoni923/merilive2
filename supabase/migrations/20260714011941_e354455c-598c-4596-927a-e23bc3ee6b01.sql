...
BEGIN
  ...
  BEGIN
    IF _auth IS NULL OR _auth <> p_caller_id THEN
      _result := jsonb_build_object('success', false, 'error', 'unauthorized');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result);
      RETURN _result;
    END IF;
    IF p_caller_id IS NULL OR p_receiver_id IS NULL THEN
      _result := jsonb_build_object('success', false, 'error', 'missing_ids');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;
    IF p_caller_id = p_receiver_id THEN
      _result := jsonb_build_object('success', false, 'error', 'cannot_call_self');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;
    ...
    IF lower(coalesce(btrim(_allow_web),'false')) NOT IN ('true','1','yes','on') THEN
      IF NOT _is_native_client THEN
        _result := jsonb_build_object('success', false, 'error', 'native_app_required', 'message', 'Private calls are available only in the MeriLive Android app.');
        INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
          VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
      END IF;
    END IF;
    ...
    IF _settings_text IS NULL OR btrim(_settings_text) = '' THEN
      _result := jsonb_build_object('success', false, 'error', 'call_rate_not_configured');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;
    BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN
      _result := jsonb_build_object('success', false, 'error', 'invalid_call_rate_config');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result,sqlstate,sqlerrm)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result,SQLSTATE,SQLERRM); RETURN _result;
    END;
    ...
    IF _caller_is_live_host THEN
      _result := jsonb_build_object('success', false, 'error', 'hosts_cannot_initiate_user_calls');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;
    ...
    IF NOT COALESCE(_receiver_ok,false) THEN
      _result := jsonb_build_object('success', false, 'error', 'receiver_not_callable_host');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    -- 2026-07-14 change: allow calling offline hosts (Chamet/WhatsApp-style FCM wake).
    -- Only 'busy' now blocks. Offline goes straight through — FCM data push wakes the app.
    SELECT lower(COALESCE(host_availability,'online')) INTO _receiver_avail
      FROM public.profiles WHERE id = p_receiver_id;
    IF _receiver_avail = 'busy' THEN
      _result := jsonb_build_object('success', false, 'error', 'host_busy', 'message', 'This host is currently busy.');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;
    ...
    IF _pc_enabled IS NOT NULL AND btrim(_pc_enabled) <> '' AND lower(btrim(_pc_enabled)) NOT IN ('true','1','yes','on') THEN
      _result := jsonb_build_object('success', false, 'error', 'private_calls_disabled');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM public.profiles WHERE id = p_caller_id AND COALESCE(is_blocked,false) = true)
      ...
      _result := jsonb_build_object('success', false, 'error', 'account_blocked');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM public.blocked_users WHERE (blocker_id = p_caller_id AND blocked_id = p_receiver_id) OR (blocker_id = p_receiver_id AND blocked_id = p_caller_id))
      ...
      _result := jsonb_build_object('success', false, 'error', 'user_blocked');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;
    ...
    END IF;
    IF _coins_per_minute IS NULL OR _coins_per_minute <= 0 THEN
      _result := jsonb_build_object('success', false, 'error', 'call_rate_not_configured', 'host_level', COALESCE(_host_level, 0));
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;
    ...
    IF _caller_balance < _coins_per_minute THEN
      _result := jsonb_build_object('success', false, 'error', 'insufficient_balance', 'required', _coins_per_minute, 'balance', _caller_balance);
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    -- 2026-07-14 change: expire not just pending/ringing, but also 'connected'
    -- rows that have gone stale (no heartbeat in ~5 min). This prevents a
    -- crashed prior session from permanently blocking new calls with
    -- 'host_busy_in_call' / 'caller_busy_in_call'.
    WITH expired AS (
      UPDATE public.private_calls
         SET status = 'missed', ended_at = now(), end_reason = 'timeout', updated_at = now()
       WHERE (
              (status IN ('pending','ringing') AND created_at < now() - make_interval(secs => _timeout_seconds))
              OR (status = 'connected' AND updated_at < now() - interval '5 minutes')
             )
    ...
    IF EXISTS (SELECT 1 FROM public.private_calls WHERE status = 'connected' AND (caller_id = p_caller_id OR host_id = p_caller_id) AND updated_at > now() - interval '5 minutes') THEN
      _result := jsonb_build_object('success', false, 'error', 'caller_busy_in_call');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    IF EXISTS (SELECT 1 FROM public.private_calls WHERE host_id = p_receiver_id AND status IN ('pending','ringing','connected') AND updated_at > now() - interval '5 minutes')
       OR EXISTS (SELECT 1 FROM public.private_calls WHERE caller_id = p_receiver_id AND status IN ('pending','ringing','connected') AND updated_at > now() - interval '5 minutes') THEN
      _result := jsonb_build_object('success', false, 'error', 'host_busy_in_call');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    IF EXISTS (SELECT 1 FROM public.private_calls WHERE status IN ('pending','ringing','connected') AND (caller_id = p_caller_id OR host_id = p_caller_id) AND updated_at > now() - interval '5 minutes') THEN
      _result := jsonb_build_object('success', false, 'error', 'caller_busy_in_call');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;
    ...
    _result := jsonb_build_object('success', true, 'call_id', _call_id, 'coins_per_minute', _coins_per_minute, 'timeout_seconds', _timeout_seconds);
    INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
      VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result);
    RETURN _result;

  EXCEPTION WHEN OTHERS THEN
    _result := jsonb_build_object('success', false, 'error', 'internal_error', 'message', SQLERRM, 'sqlstate', SQLSTATE);
    BEGIN
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result,sqlstate,sqlerrm)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result,SQLSTATE,SQLERRM);
