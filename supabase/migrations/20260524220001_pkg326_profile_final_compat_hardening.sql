-- Pkg326 final pass: keep strict profile protection while allowing only safe first-time auth-owned setup fields.
...
DECLARE
  ...
  _jwt_email text := lower(nullif(current_setting('request.jwt.claim.email', true), ''));
  _expected_phone text;
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    IF NEW.is_verified IS DISTINCT FROM OLD.is_verified THEN
      IF NOT (auth.uid() = OLD.id AND COALESCE(OLD.is_verified, false) = false AND NEW.is_verified = true) THEN
    ...
    IF NEW.email IS DISTINCT FROM OLD.email THEN
      IF NOT (auth.uid() = OLD.id AND OLD.email IS NULL AND lower(NEW.email) = _jwt_email) THEN
        RAISE EXCEPTION 'Direct modification of email is not allowed';
      END IF;
    END IF;
    IF NEW.phone_number IS DISTINCT FROM OLD.phone_number THEN
      _expected_phone := CASE WHEN _jwt_email LIKE 'phone_%@meri.local' THEN regexp_replace(split_part(_jwt_email, '@', 1), '^phone_', '') ELSE NULL END;
      IF NOT (auth.uid() = OLD.id AND OLD.phone_number IS NULL AND regexp_replace(COALESCE(NEW.phone_number,''), '[^0-9]', '', 'g') = _expected_phone) THEN
    ...
    IF NEW.phone_verified IS DISTINCT FROM OLD.phone_verified THEN
      IF NOT (auth.uid() = OLD.id AND COALESCE(OLD.phone_verified, false) = false AND NEW.phone_verified = true AND lower(COALESCE(NEW.email, _jwt_email)) LIKE 'phone_%@meri.local') THEN
        RAISE EXCEPTION 'Direct modification of phone_verified is not allowed';
      END IF;
    END IF;
    IF NEW.device_id IS DISTINCT FROM OLD.device_id THEN
      IF NOT (auth.uid() = OLD.id AND OLD.device_id IS NULL AND NEW.device_id IS NOT NULL AND NEW.device_id ~ '^device_[A-Za-z0-9_:-]{6,128}$') THEN
