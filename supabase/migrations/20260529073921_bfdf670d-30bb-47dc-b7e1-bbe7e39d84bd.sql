-- Pkg381: Diagnostic for "Call Failed - Please try again"
-- Wrap start_private_call so we capture every entry + return + exception.
-- Lets us see the exact failure reason after user retries from APK.

CREATE TABLE IF NOT EXISTS public.private_call_diag (
  id bigserial PRIMARY KEY,
  occurred_at timestamptz NOT NULL DEFAULT now(),
  caller_id uuid,
  receiver_id uuid,
  auth_uid uuid,
  client_platform text,
  user_agent text,
  result jsonb,
  sqlstate text,
  sqlerrm text
);

GRANT SELECT, INSERT ON public.private_call_diag TO authenticated;
GRANT ALL ON public.private_call_diag TO service_role;
GRANT USAGE, SELECT ON SEQUENCE public.private_call_diag_id_seq TO authenticated, service_role;

ALTER TABLE public.private_call_diag ENABLE ROW LEVEL SECURITY;
CREATE POLICY "diag_admin_or_self_read" ON public.private_call_diag
  FOR SELECT TO authenticated
  USING (auth.uid() = caller_id OR public.is_admin(auth.uid()));
CREATE POLICY "diag_system_insert" ON public.private_call_diag
  FOR INSERT TO authenticated WITH CHECK (true);

-- Rebuild start_private_call with diagnostic INSERTs around every early return.
CREATE OR REPLACE FUNCTION public.start_private_call(
  p_caller_id uuid,
  p_receiver_id uuid,
  p_call_type text DEFAULT 'video',
  p_context_stream_id uuid DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE
  _caller_balance bigint;
  _coins_per_minute bigint;
  _host_level integer;
  _host_custom_rate integer;
  _call_id uuid;
  _settings_text text;
  _settings jsonb;
  _level_rates jsonb;
  _default_rate integer;
  _timeout_seconds integer := 60;
  _caller_is_live_host boolean;
  _receiver_ok boolean;
  _receiver_avail text;
  _rate_entry jsonb;
  _pc_enabled text;
  _client_platform text;
  _client_ua text;
  _allow_web text;
  _is_native_client boolean := false;
  _min_rate numeric := 0;
  _max_rate numeric := 0;
  _min_custom_level integer := 6;
  _custom_is_level_rate boolean := false;
  _result jsonb;
  _auth uuid := auth.uid();
BEGIN
  BEGIN
    _client_platform := lower(coalesce(current_setting('request.headers', true)::json->>'x-client-platform',''));
  EXCEPTION WHEN OTHERS THEN _client_platform := ''; END;
  BEGIN
    _client_ua := lower(coalesce(current_setting('request.headers', true)::json->>'user-agent',''));
  EXCEPTION WHEN OTHERS THEN _client_ua := ''; END;

  -- Wrap entire logic so any exception is captured for diagnostics.
  BEGIN
    IF _auth IS NULL OR _auth <> p_caller_id THEN
      _result := jsonb_build_object('success', false, 'error', 'unauthorized');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result);
      RETURN _result;
    END IF;
    IF p_caller_id IS NULL OR p_receiver_id IS NULL THEN
      _result := jsonb_build_object('success', false, 'error', 'missing_ids');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;
    IF p_caller_id = p_receiver_id THEN
      _result := jsonb_build_object('success', false, 'error', 'cannot_call_self');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    _is_native_client := (
      _client_platform IN ('android-native','ios-native')
      OR position('merilive-android-native' in _client_ua) > 0
      OR position('merilive-ios-native' in _client_ua) > 0
      OR position('capacitor' in _client_ua) > 0
    );

    SELECT setting_value INTO _allow_web FROM public.app_settings WHERE setting_key = 'allow_web_calls' LIMIT 1;
    IF lower(coalesce(btrim(_allow_web),'false')) NOT IN ('true','1','yes','on') THEN
      IF NOT _is_native_client THEN
        _result := jsonb_build_object('success', false, 'error', 'native_app_required', 'message', 'Private calls are available only in the MeriLive Android app.');
        INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
          VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
      END IF;
    END IF;

    SELECT setting_value INTO _settings_text FROM public.app_settings WHERE setting_key = 'call_rates' LIMIT 1;
    IF _settings_text IS NULL OR btrim(_settings_text) = '' THEN
      _result := jsonb_build_object('success', false, 'error', 'call_rate_not_configured');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;
    BEGIN
      _settings := _settings_text::jsonb;
      _default_rate := NULLIF((_settings->>'default_rate')::integer, 0);
      _timeout_seconds := GREATEST(15, LEAST(120, COALESCE((_settings->>'call_timeout_seconds')::integer, 60)));
      _level_rates := COALESCE(_settings->'level_rates', '[]'::jsonb);
      _min_rate := COALESCE(NULLIF((_settings->>'min_rate')::numeric, 0), 0);
      _max_rate := COALESCE(NULLIF((_settings->>'max_rate')::numeric, 0), 0);
      _min_custom_level := COALESCE((_settings->>'min_level_for_custom_rate')::integer, 6);
    EXCEPTION WHEN OTHERS THEN
      _result := jsonb_build_object('success', false, 'error', 'invalid_call_rate_config');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result,sqlstate,sqlerrm)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result,SQLSTATE,SQLERRM); RETURN _result;
    END;

    PERFORM set_config('app.bypass_profile_protection', 'true', true);
    PERFORM 1 FROM public.profiles WHERE id IN (p_caller_id, p_receiver_id) FOR UPDATE;

    SELECT EXISTS (
      SELECT 1 FROM public.profiles
       WHERE id = p_caller_id AND is_host = true
         AND lower(COALESCE(host_status,'')) = 'approved'
         AND COALESCE(is_face_verified,false) = true
    ) INTO _caller_is_live_host;
    IF _caller_is_live_host THEN
      _result := jsonb_build_object('success', false, 'error', 'hosts_cannot_initiate_user_calls');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    SELECT EXISTS (
      SELECT 1 FROM public.profiles
       WHERE id = p_receiver_id AND is_host = true
         AND lower(COALESCE(host_status,'')) = 'approved'
         AND COALESCE(is_face_verified,false) = true
    ) INTO _receiver_ok;
    IF NOT COALESCE(_receiver_ok,false) THEN
      _result := jsonb_build_object('success', false, 'error', 'receiver_not_callable_host');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    SELECT lower(COALESCE(host_availability,'online')) INTO _receiver_avail
      FROM public.profiles WHERE id = p_receiver_id;
    IF _receiver_avail = 'offline' THEN
      _result := jsonb_build_object('success', false, 'error', 'host_offline', 'message', 'This host is currently offline.');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    ELSIF _receiver_avail = 'busy' THEN
      _result := jsonb_build_object('success', false, 'error', 'host_busy', 'message', 'This host is currently busy.');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    SELECT setting_value INTO _pc_enabled FROM public.app_settings WHERE setting_key = 'private_calls_enabled' LIMIT 1;
    IF _pc_enabled IS NOT NULL AND btrim(_pc_enabled) <> '' AND lower(btrim(_pc_enabled)) NOT IN ('true','1','yes','on') THEN
      _result := jsonb_build_object('success', false, 'error', 'private_calls_disabled');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    IF EXISTS (SELECT 1 FROM public.profiles WHERE id = p_caller_id AND COALESCE(is_blocked,false) = true)
       OR EXISTS (SELECT 1 FROM public.profiles WHERE id = p_receiver_id AND COALESCE(is_blocked,false) = true) THEN
      _result := jsonb_build_object('success', false, 'error', 'account_blocked');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    IF EXISTS (SELECT 1 FROM public.blocked_users WHERE (blocker_id = p_caller_id AND blocked_id = p_receiver_id) OR (blocker_id = p_receiver_id AND blocked_id = p_caller_id))
       OR EXISTS (SELECT 1 FROM public.user_blocks WHERE (blocker_id = p_caller_id AND blocked_id = p_receiver_id) OR (blocker_id = p_receiver_id AND blocked_id = p_caller_id)) THEN
      _result := jsonb_build_object('success', false, 'error', 'user_blocked');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    SELECT COALESCE(host_level, 0), call_rate_per_minute
      INTO _host_level, _host_custom_rate
      FROM public.profiles WHERE id = p_receiver_id;

    IF COALESCE(_host_custom_rate, 0) > 0 THEN
      IF _level_rates IS NOT NULL AND jsonb_typeof(_level_rates) = 'array' THEN
        SELECT EXISTS (
          SELECT 1 FROM jsonb_array_elements(_level_rates) lr
          WHERE (lr->>'rate')::numeric = _host_custom_rate
        ) INTO _custom_is_level_rate;
      END IF;

      IF _custom_is_level_rate
         OR (
           COALESCE(_host_level, 0) >= _min_custom_level
           AND (_min_rate <= 0 OR _host_custom_rate >= _min_rate)
           AND (_max_rate <= 0 OR _host_custom_rate <= _max_rate)
         ) THEN
        _coins_per_minute := _host_custom_rate;
      END IF;
    END IF;

    IF (_coins_per_minute IS NULL OR _coins_per_minute <= 0) AND _level_rates IS NOT NULL AND jsonb_typeof(_level_rates) = 'array' THEN
      FOR _rate_entry IN SELECT * FROM jsonb_array_elements(_level_rates) LOOP
        IF (_rate_entry->>'level')::integer = COALESCE(_host_level, 0) THEN
          _coins_per_minute := NULLIF((_rate_entry->>'rate')::integer, 0);
          EXIT;
        END IF;
      END LOOP;
    END IF;

    IF _coins_per_minute IS NULL OR _coins_per_minute <= 0 THEN
      _coins_per_minute := _default_rate;
    END IF;
    IF _coins_per_minute IS NULL OR _coins_per_minute <= 0 THEN
      _result := jsonb_build_object('success', false, 'error', 'call_rate_not_configured', 'host_level', COALESCE(_host_level, 0));
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    SELECT COALESCE(coins,0) INTO _caller_balance FROM public.profiles WHERE id = p_caller_id FOR UPDATE;
    IF _caller_balance < _coins_per_minute THEN
      _result := jsonb_build_object('success', false, 'error', 'insufficient_balance', 'required', _coins_per_minute, 'balance', _caller_balance);
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    WITH expired AS (
      UPDATE public.private_calls
         SET status = 'missed', ended_at = now(), end_reason = 'timeout', updated_at = now()
       WHERE status IN ('pending', 'ringing')
         AND created_at < now() - make_interval(secs => _timeout_seconds)
         AND (caller_id IN (p_caller_id, p_receiver_id) OR host_id IN (p_caller_id, p_receiver_id))
       RETURNING caller_id, host_id, id
    )
    UPDATE public.profiles p
       SET is_in_call = false, current_call_id = NULL, updated_at = now()
      FROM expired e
     WHERE p.id IN (e.caller_id, e.host_id)
       AND p.current_call_id = e.id;

    UPDATE public.profiles p
       SET is_in_call = false, current_call_id = NULL, updated_at = now()
     WHERE p.id IN (p_caller_id, p_receiver_id)
       AND p.current_call_id IS NOT NULL
       AND NOT EXISTS (
         SELECT 1 FROM public.private_calls pc
          WHERE pc.id = p.current_call_id
            AND pc.status IN ('pending', 'ringing', 'connected')
       );

    IF EXISTS (SELECT 1 FROM public.private_calls WHERE status = 'connected' AND (caller_id = p_caller_id OR host_id = p_caller_id)) THEN
      _result := jsonb_build_object('success', false, 'error', 'caller_busy_in_call');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    IF EXISTS (SELECT 1 FROM public.private_calls WHERE host_id = p_receiver_id AND status IN ('pending', 'ringing', 'connected'))
       OR EXISTS (SELECT 1 FROM public.private_calls WHERE caller_id = p_receiver_id AND status IN ('pending', 'ringing', 'connected')) THEN
      _result := jsonb_build_object('success', false, 'error', 'host_busy_in_call');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    IF EXISTS (SELECT 1 FROM public.private_calls WHERE status IN ('pending', 'ringing', 'connected') AND (caller_id = p_caller_id OR host_id = p_caller_id)) THEN
      _result := jsonb_build_object('success', false, 'error', 'caller_busy_in_call');
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result); RETURN _result;
    END IF;

    INSERT INTO public.private_calls (caller_id, host_id, stream_id, status, started_at, coins_per_minute)
    VALUES (p_caller_id, p_receiver_id, p_context_stream_id, 'ringing', now(), _coins_per_minute)
    RETURNING id INTO _call_id;

    UPDATE public.profiles
       SET is_in_call = true, current_call_id = _call_id, updated_at = now()
     WHERE id IN (p_caller_id, p_receiver_id);

    INSERT INTO public.call_events (caller_id, receiver_id, call_type, status, call_id, event_type, event_data)
    VALUES (p_caller_id, p_receiver_id, COALESCE(NULLIF(p_call_type, ''), 'video'), 'ringing', _call_id, 'call_started', jsonb_build_object('coins_per_minute', _coins_per_minute, 'timeout_seconds', _timeout_seconds));

    _result := jsonb_build_object('success', true, 'call_id', _call_id, 'coins_per_minute', _coins_per_minute, 'timeout_seconds', _timeout_seconds);
    INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result)
      VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result);
    RETURN _result;

  EXCEPTION WHEN OTHERS THEN
    -- Catch any unexpected exception so JS gets a clear jsonb back instead of HTTP error.
    _result := jsonb_build_object('success', false, 'error', 'internal_error', 'message', SQLERRM, 'sqlstate', SQLSTATE);
    BEGIN
      INSERT INTO public.private_call_diag(caller_id,receiver_id,auth_uid,client_platform,user_agent,result,sqlstate,sqlerrm)
        VALUES (p_caller_id,p_receiver_id,_auth,_client_platform,_client_ua,_result,SQLSTATE,SQLERRM);
    EXCEPTION WHEN OTHERS THEN NULL; END;
    RETURN _result;
  END;
END;
$function$;

GRANT EXECUTE ON FUNCTION public.start_private_call(uuid,uuid,text,uuid) TO authenticated;