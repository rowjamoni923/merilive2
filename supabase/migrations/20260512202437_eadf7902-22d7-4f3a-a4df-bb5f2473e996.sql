...
DECLARE
  ...
  v_notes text;
BEGIN
  IF p_user_id IS NULL OR p_product_id IS NULL OR trim(p_product_id) = '' OR p_purchase_token IS NULL OR trim(p_purchase_token) = '' THEN
  ...
  v_notes := 'Server-verified Google Play purchase. Product: ' || p_product_id || '. Order: ' || COALESCE(v_order_id, 'N/A');
  ...
  VALUES (p_user_id, p_user_id, v_pkg.total_coins, 'google_play', 'completed', 'Google Play purchase: ' || p_product_id);
...
EXCEPTION WHEN unique_violation THEN
  SELECT COALESCE(coins, 0) INTO v_balance_after FROM public.profiles WHERE id = p_user_id;
  RETURN jsonb_build_object('success', true, 'alreadyProcessed', true, 'coins', v_pkg.total_coins, 'newBalance', COALESCE(v_balance_after, 0));
END;
$$;