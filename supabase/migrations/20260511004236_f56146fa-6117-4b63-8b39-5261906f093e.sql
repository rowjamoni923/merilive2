...
VALUES ('face_verification_auto_approve_enabled', 'false')
...
BEGIN
  ...
  SELECT * INTO v_sub FROM public.face_verification_submissions WHERE id = p_submission_id;
  IF NOT FOUND THEN
  ...
  v_face_url := coalesce(v_sub.face_image_url, v_sub.selfie_url, v_sub.front_url);
...
COMMENT ON FUNCTION public.service_auto_finalize_face_verification(uuid) IS
  'After Edge writes ai_analysis.rekognition, optionally auto-approve if app_settings.face_verification_auto_approve_enabled is true. service_role only.';