...
BEGIN
  ...
  -- Prefer still images for profile verification evidence. face_image_url can be a video in the current app flow.
...
BEGIN
  ...
  -- Store the best still-image evidence on the profile, not the video URL.
  ...
  RETURN jsonb_build_object('success', true, 'gender', v_final, 'verification_type', v_vt);
END;
$$;
