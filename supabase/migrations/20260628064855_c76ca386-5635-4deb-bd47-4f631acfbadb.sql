...
BEGIN
  ...
  -- Strong identity: photo↔live AND face-video↔live both ≥ 85% (industry-grade
  -- biometric confidence). Used to relax soft signals like occlusion / mid-range
  -- gender / face confidence — these often trip on hair, lighting, or beauty
  -- filters even when the person is unambiguously the same human.
  v_strong_identity := v_passive_scan
    AND v_evidence_complete
    AND v_evidence_same_person
    AND NOT v_identity_mismatch
    AND v_photo_live >= 85
    AND v_face_video_live >= 85;
  ...
  -- Owner policy 2026-06-28: relax soft thresholds so ~95% of genuine new accounts
  -- pass. Strong-identity submissions (photo+video+live all match the same person
  -- ≥85%) skip the gender/face confidence floors entirely — those signals were
  -- false-failing real users behind beauty filters and low light.
  IF NOT v_strong_identity THEN
    IF v_gender_conf < 60 OR v_face_conf < 55 THEN
  ...
  -- Occlusion is now a SOFT signal: only block when identity proof is weak.
  -- Hair across forehead or glasses commonly push occlusion >95% even when the
  -- same-person match is 95%+ — that was the main false-retry driver.
  ...
  SET status = 'approved',
      ...
      admin_notes = CASE
        WHEN trim(coalesce(admin_notes, '')) = '' THEN
          '[auto] service_auto_finalize_face_verification: approved (relaxed gates 2026-06-28).'
        ELSE
          trim(admin_notes) || E'\n[auto] service_auto_finalize_face_verification: approved (relaxed gates 2026-06-28, strong_identity=' || v_strong_identity::text || ').'
      END,
      updated_at = now()
