...
BEGIN
  ...
  WITH followed AS (
    ...
    UNION
    -- Random calls (homepage must also reflect these as Busy)
    SELECT DISTINCT rcs.host_id
    FROM public.random_call_sessions rcs
    WHERE rcs.status IN ('pending','ringing','connected','active','matched')
      AND rcs.ended_at IS NULL
      AND rcs.host_id IS NOT NULL
  ), eligible AS (
    SELECT
      p.*,
      ll.stream_id,
