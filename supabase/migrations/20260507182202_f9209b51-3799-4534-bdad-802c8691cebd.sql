...
-- 1) Self-recharge: agency diamond balance first, then trader wallet
...
-- 2) agencies_public + diamond_balance
...
CREATE POLICY "coin_trader_transfers_select_own"
  ON public.coin_trader_transfers FOR SELECT TO authenticated
...
CREATE POLICY "coin_trader_transfers_no_insert"
  ON public.coin_trader_transfers FOR INSERT TO authenticated
  WITH CHECK (false);

GRANT SELECT ON public.coin_trader_transfers TO authenticated;

-- 5) RPC: transfer to user
...
BEGIN
  ...
  IF COALESCE((j->>'success')::boolean, false) THEN
    INSERT INTO public.coin_trader_transfers (
      user_id, counterparty_agency_id, amount, transfer_type, status
