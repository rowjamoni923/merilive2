...
-- ============================================================
-- Bug #5 + #6: process_gift_transaction
-- ============================================================
...
BEGIN
  ...
  -- Bug #6: tag every balance change in balance_audit_log
  ...
  -- Bug #5: acquire BOTH locks in deterministic UUID order to prevent
  -- A→B / B→A mutual-gift deadlocks. We pre-lock with no projection, then
  -- re-select with full columns (PG remembers the row lock for this tx).
  IF p_sender_id < p_receiver_id THEN
...
-- ============================================================
-- Bug #6: tag game RPCs in audit log
-- ============================================================
...
-- 3-arg wrapper just delegates; tagging happens in the 4-arg path it calls.
