...
-- Fix: Remove SET role = postgres which is not allowed in Supabase
-- Recreate distribute_period_rewards as SECURITY DEFINER (bypasses RLS automatically)
...
-- Also recreate auto_distribute without SET role
...
BEGIN
  ...
  IF v_results = '' THEN
    v_results := 'No distributions needed (DOW: ' || v_utc_dow || ', Day: ' || v_utc_day || ')';
  END IF;
