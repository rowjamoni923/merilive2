-- Pkg370: make privilege equip persistent across shop, level, VIP and mixed-source animation items.

ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_equipped_entrance_id_fkey;
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_equipped_bubble_id_fkey;
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_equipped_vehicle_id_fkey;
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_equipped_entry_banner_id_fkey;
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_equipped_entry_name_bar_id_fkey;
...
BEGIN
  IF _slot IN ('frame', 'portrait_frame') THEN
    ...
    END IF;
  ELSIF _slot IN ('entrance', 'entrance_effect', 'entry_banner') THEN
    _updates := _updates || jsonb_build_object('equipped_entrance_id', _item_id, 'equipped_entry_banner_id', _item_id);
    ...
    END IF;
    IF _profile.equipped_entry_banner_id IS NOT NULL AND _profile.equipped_entry_banner_id IS DISTINCT FROM _item_id THEN
      _updates := _updates || jsonb_build_object('previous_entry_banner_id', _profile.equipped_entry_banner_id);
    END IF;
  ELSIF _slot IN ('entry_bar', 'entry_name_bar', 'entry_bar_effect') THEN
  ...
  ELSIF _slot IN ('vehicle', 'vehicle_entrance') THEN
    ...
    END IF;
  ELSIF _slot IN ('medal', 'badge', 'vip_medal') THEN
