...
DECLARE
  ...
  v_sender_is_agency_owner BOOLEAN := false;
  v_sender_is_helper     BOOLEAN := false;
BEGIN
  SELECT
    COALESCE(p.is_host, false),
    COALESCE(p.is_agency_owner, false),
    EXISTS (
      SELECT 1
  ...
  INTO v_sender_is_host, v_sender_is_agency_owner, v_sender_is_helper
  FROM public.profiles p
  WHERE p.id = p_host_id;

  -- Owner-locked rule: only real verified hosts are subject to deductions/bans.
  -- Agency owners and verified top-up helpers are support/payment roles, so they
  -- can share numbers. Agency-managed hosts still have is_host=true and remain blocked.
  IF COALESCE(v_sender_is_host, false) IS NOT TRUE
     OR COALESCE(v_sender_is_agency_owner, false) IS TRUE
     OR COALESCE(v_sender_is_helper, false) IS TRUE THEN
    RETURN jsonb_build_object(
      'success', true,
      'skipped', true,
      'reason', CASE
        WHEN COALESCE(v_sender_is_agency_owner, false) THEN 'sender_is_agency_owner'
