...
BEGIN
  ...
  BEGIN
    ...
    -- LC-CTX 2026-07-16: best-effort validate the linked stream so
    -- private_calls.stream_id is only populated when the host is actually
    -- live on that stream. Never fails the call — bad/stale ids silently
    -- become NULL.
    IF p_context_stream_id IS NOT NULL
       AND NOT EXISTS (
