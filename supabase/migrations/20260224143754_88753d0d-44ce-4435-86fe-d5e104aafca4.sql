-- Fix recover_session_by_device: use extensions.gen_random_bytes instead of unqualified gen_random_bytes
...
BEGIN
  ...
  -- Log recovery attempt (use gen_random_uuid instead of gen_random_bytes to avoid pgcrypto dependency)
  INSERT INTO recovery_tokens (user_id, device_id, token)
  VALUES (v_profile.id, p_device_id, replace(gen_random_uuid()::text, '-', ''));
