...
-- Fix type mismatch: app_uid is varchar, frame_id is uuid
...
BEGIN
  ...
  RETURN QUERY
  WITH host_ids AS (SELECT p.id FROM profiles p WHERE p.is_host = true),
  gift_earnings AS (
    SELECT gt.receiver_id AS uid, COALESCE(SUM(FLOOR(gt.coin_amount * 0.6)), 0)::bigint AS beans
    FROM gift_transactions gt INNER JOIN host_ids h ON h.id = gt.receiver_id
    WHERE gt.created_at >= v_start_date GROUP BY gt.receiver_id
  ),
  call_earnings AS (
    SELECT pc.host_id AS uid, COALESCE(SUM(pc.host_earnings_amount), 0)::bigint AS beans
    FROM private_calls pc INNER JOIN host_ids h ON h.id = pc.host_id
    WHERE pc.created_at >= v_start_date AND pc.status = 'completed' GROUP BY pc.host_id
  ),
  combined AS (
...
BEGIN
  ...
  WITH game_stats AS (
    ...
    FROM game_transactions gt WHERE gt.created_at >= v_start_date
    GROUP BY gt.user_id HAVING COUNT(*) > 0
  )
  SELECT p.id, p.display_name, p.app_uid, p.avatar_url, p.country_flag, p.host_level, p.user_level, p.frame_id,
         gs.total_bets AS stat_value
  FROM game_stats gs INNER JOIN profiles p ON p.id = gs.uid
  ORDER BY gs.total_bets DESC LIMIT 50;
END; $$;
