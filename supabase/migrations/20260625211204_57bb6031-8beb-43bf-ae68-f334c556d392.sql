...
BEGIN
  ...
  -- v_expected_gender is retained only to derive verification_type / is_host
  -- downstream. It is NOT used to reject or hold submissions (owner policy
  -- 2026-06-26: gender mismatch is never a reject or manual-review reason).
  ...
  -- HARD AUTO-REJECT #1: duplicate face (same person, second account)
  ...
  -- Gender-mismatch auto-reject INTENTIONALLY REMOVED (owner policy 2026-06-26).
  ...
  -- Identity mismatch (photo+video+live not the same person) → soft retry, handled
  -- by the edge function which sets status='needs_retry'. Here we just refuse to
  -- auto-approve, no DB-side hard reject.
  IF v_passive_scan AND v_identity_mismatch THEN
    RETURN jsonb_build_object(
      'success', false,
      'reason', 'photo_video_live_identity_mismatch',
  ...
  -- Gender gates (invalid_final_gender, account_gender_mismatch) INTENTIONALLY
  -- REMOVED. Auto-approve proceeds regardless of detected gender.
