...
BEGIN
  ...
  -- Temporarily switch role to bypass the protect_sensitive_columns_trigger
  _old_role := current_setting('role', true);
  PERFORM set_config('role', 'service_role', true);

  -- Update user profile (now bypasses trigger protection)
  UPDATE profiles
  SET is_agency_owner = true, agency_id = _new_agency_id
  WHERE id = _owner_id;

  -- Restore original role
  PERFORM set_config('role', _old_role, true);
