...
-- Fix: Update request_agency_withdrawal to use beans_balance instead of wallet_balance
-- as per the "100% Flush" policy where all host earnings go to beans_balance
...
BEGIN
  -- Get current agency beans_balance (this is where host earnings are stored)
  SELECT COALESCE(beans_balance, 0) INTO _agency_balance
  ...
  IF _agency_balance < _amount THEN
    RETURN json_build_object(
      ...
      'available_balance', _agency_balance,
  ...
  -- Deduct from agency beans_balance (not wallet_balance)
  UPDATE agencies 
