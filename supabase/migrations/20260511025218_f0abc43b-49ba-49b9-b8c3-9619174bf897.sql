DROP FUNCTION IF EXISTS public.can_initiate_private_call(uuid, uuid);
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM public.profiles WHERE id = p_caller_id AND COALESCE(is_blocked, false) = true
  ) OR EXISTS (
    SELECT 1 FROM public.profiles WHERE id = p_host_id AND COALESCE(is_blocked, false) = true
  ) THEN
  ...
  SELECT COALESCE(is_in_call, false) INTO _host_in_call
  ...
  ELSE
    IF NOT EXISTS (
      SELECT 1
      FROM public.live_streams ls
      WHERE ls.id = p_context_stream_id
        AND ls.host_id = p_host_id
        AND ls.ended_at IS NULL
        AND COALESCE(ls.is_active, true) = true
        AND lower(COALESCE(ls.status, 'active')) = 'active'
    ) THEN
...
COMMENT ON FUNCTION public.can_initiate_private_call(uuid, uuid, uuid) IS
'Preflight for private calls. Optional p_context_stream_id: when set, host may be live on that stream only (in-room call).';

GRANT EXECUTE ON FUNCTION public.can_initiate_private_call(uuid, uuid, uuid) TO authenticated;
...
BEGIN
  ...
  IF EXISTS (
    ...
    SELECT 1 FROM public.profiles WHERE id = p_receiver_id AND COALESCE(is_blocked, false) = true
  ) THEN
  ...
  IF EXISTS (
    SELECT 1 FROM public.profiles WHERE id = p_receiver_id AND COALESCE(is_in_call, false) = true
  ) THEN
  ...
  IF p_context_stream_id IS NULL THEN
    IF EXISTS (
      SELECT 1
      FROM public.live_streams ls
      WHERE ls.host_id = p_receiver_id
        AND ls.ended_at IS NULL
        AND COALESCE(ls.is_active, true) = true
        AND lower(COALESCE(ls.status, 'active')) = 'active'
    ) THEN
  ...
  ELSE
    IF NOT EXISTS (
      ...
      RETURN jsonb_build_object('success', false, 'error', 'invalid_stream_context');
    END IF;
  END IF;
