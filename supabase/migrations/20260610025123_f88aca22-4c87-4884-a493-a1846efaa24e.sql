...
DECLARE
  ...
  helper_final_balance bigint := 0;
  agency_final_balance bigint := 0;
...
BEGIN
  ...
  helper_final_balance := CASE
    WHEN helper_rec.id IS NULL THEN 0
    WHEN helper_deducted > 0 THEN helper_balance_after
    ELSE COALESCE(helper_rec.wallet_balance, 0)
  END;

  agency_final_balance := CASE
    WHEN agency_rec.id IS NULL THEN 0
    WHEN agency_deducted > 0 THEN agency_balance_after
    ELSE COALESCE(agency_rec.diamond_balance, 0)
  END;
  ...
  RETURN jsonb_build_object(
    ...
    'new_coins', new_coins,
    'new_wallet_balance', helper_final_balance,
    'new_helper_wallet_balance', helper_final_balance,
    'new_agency_balance', agency_final_balance,
    'available_balance', helper_final_balance + agency_final_balance,
    'source', CASE
