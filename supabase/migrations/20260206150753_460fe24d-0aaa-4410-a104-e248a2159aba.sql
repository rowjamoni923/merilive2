-- Update distribute_period_rewards to handle top_gifters category
...
BEGIN
  ...
  -- Process top_gifters category (users who send the most gifts)
...
-- Update auto_distribute to include top_gifters
...
BEGIN
  FOREACH v_cat IN ARRAY v_categories LOOP
    -- Daily: distribute after midnight
    IF EXTRACT(HOUR FROM v_now) = 0 THEN
    ...
    -- Weekly: distribute on Monday
    IF EXTRACT(DOW FROM v_now) = 1 AND EXTRACT(HOUR FROM v_now) = 0 THEN
    ...
    -- Monthly: distribute on 1st
    IF EXTRACT(DAY FROM v_now) = 1 AND EXTRACT(HOUR FROM v_now) = 0 THEN
