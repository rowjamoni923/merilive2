...
BEGIN
  ...
  -- Allow service_role (edge functions), postgres (sweep cron), and supabase_admin (migrations)
  ...
  END IF;
  -- Delegate to internal worker so we don't duplicate 200 lines
  RETURN public._service_auto_finalize_face_verification_impl(p_submission_id);
END;
$function$;

-- Internal implementation (moved from above; no auth check)
...
BEGIN
  ...
  SET status = 'approved', verification_type = v_vt, reviewed_at = now(),
      admin_notes = CASE
        WHEN trim(coalesce(admin_notes, '')) = '' THEN '[auto v3 2026-06-29] approved (strong=' || v_strong_identity::text || ' super_strong=' || v_super_strong::text || ').'
