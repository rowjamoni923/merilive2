...
BEGIN
  ...
  -- Sanitize host_photos gallery (Step-2 three photos) — published for EVERYONE
  -- (user or host) on approval, never before approval.
  ...
  -- Step-1 Basic Info (name / age / language) → profile, for everyone
  ...
  SET status = 'approved',
      ...
      admin_notes = CASE
        WHEN trim(coalesce(admin_notes, '')) = '' THEN
          '[auto] service_auto_finalize_face_verification: approved — name/age/language + profile photo + gallery photos published to profile.'
        ELSE
          trim(admin_notes) || E'\n[auto] service_auto_finalize_face_verification: approved — name/age/language + profile photo + gallery photos published to profile.'
      END,
      updated_at = now()
