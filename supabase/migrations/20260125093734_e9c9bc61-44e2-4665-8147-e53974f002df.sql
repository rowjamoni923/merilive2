...
-- Drop existing function and recreate with proper types
DROP FUNCTION IF EXISTS public.exchange_agency_beans_to_diamonds(uuid, numeric, numeric, numeric);
DROP FUNCTION IF EXISTS public.exchange_agency_beans_to_diamonds(uuid, bigint, bigint, bigint);

-- Create function with correct parameter types matching how it's called from frontend
...
BEGIN
  ...
  -- Update agency balances atomically
  UPDATE agencies 
  SET 
...
GRANT EXECUTE ON FUNCTION public.exchange_agency_beans_to_diamonds(uuid, bigint, bigint, bigint) TO anon;
