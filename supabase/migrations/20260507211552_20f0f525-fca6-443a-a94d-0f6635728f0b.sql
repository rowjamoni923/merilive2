-- Section 18 gap fix: seed FAQ + add active_device_id to update_profile RPC

INSERT INTO public.app_content (id, type, title, content, is_published, created_at, updated_at)
SELECT gen_random_uuid(), 'faq', 'FAQ',
  E'## Frequently asked questions\n\n### Account\n- Use **Settings → My account** to view your UID and linked email.\n\n### Privacy\n- **Hide location** hides your city from other profiles.\n\n### Support\n- **Report a problem** opens a ticket for our team.',
  true, now(), now()
WHERE NOT EXISTS (SELECT 1 FROM public.app_content c WHERE c.type = 'faq' LIMIT 1);
...
BEGIN
  ...
  IF p_patch ? 'active_device_id' THEN
    v_device := nullif(trim(p_patch->>'active_device_id'), '');
    IF v_device IS NOT NULL AND char_length(v_device) > 128 THEN
      RETURN jsonb_build_object('success', false, 'error', 'Invalid device id');
    END IF;
  END IF;
  ...
  UPDATE public.profiles SET
    ...
    active_device_id = CASE WHEN p_patch ? 'active_device_id' THEN v_device ELSE active_device_id END,
