...
-- Update admin_process_withdrawal to dynamically calculate diamond_reward if it's 0 or null
...
BEGIN
  ...
  ELSIF _action = 'complete' THEN
    ...
    IF _withdrawal.assigned_helper_id IS NOT NULL THEN
      -- Calculate diamond_reward from payment_details.usd_amount if not already set
      _diamond_reward := COALESCE(NULLIF(_withdrawal.diamond_reward, 0), 0);
      
      -- If diamond_reward is 0, calculate from payment_details.usd_amount (1 USD = 100 diamonds)
      IF _diamond_reward = 0 THEN
        _usd_amount := COALESCE(
          (_withdrawal.payment_details->>'usd_amount')::NUMERIC,
          _withdrawal.amount / 10000.0  -- Fallback: 10000 beans = $1
        );
        _diamond_reward := ROUND(_usd_amount * 100, 0); -- 1 USD = 100 diamonds
      END IF;
