-- Pkg325 Wallet pass-2b: prevent partial deduction on insufficient helper/trader transfers
...
BEGIN
  ...
  IF helper_rec IS NOT NULL THEN SELECT wallet_balance INTO new_wallet FROM public.topup_helpers WHERE id = helper_rec.id; END IF;

  RETURN jsonb_build_object('success', true, 'helper_deducted', helper_deducted, 'agency_deducted', agency_deducted, 'new_coins', new_coins, 'new_wallet_balance', new_wallet, 'new_agency_balance', CASE WHEN agency_rec IS NOT NULL THEN (SELECT diamond_balance FROM public.agencies WHERE id = agency_rec.id) ELSE NULL END);
END;
$$;

CREATE OR REPLACE FUNCTION public.helper_transfer_coins_to_user(_sender_id uuid, _receiver_id uuid, _amount bigint, _sender_type text DEFAULT 'trader_to_user')
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  IF NOT public.is_approved_topup_trader(_sender_id) THEN RETURN jsonb_build_object('success', false, 'error', 'Only approved L1-L5 helper traders can transfer'); END IF;
...
CREATE OR REPLACE FUNCTION public.helper_transfer_diamonds_to_agency(_sender_id uuid, _target_agency_id uuid, _amount bigint, _sender_type text DEFAULT 'trader_to_agency')
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
