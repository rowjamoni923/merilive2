-- Pkg370: Admin balance adjust now writes an instant in-app notification
-- for the affected user. Delivered via Supabase Realtime (notifications
-- table IS in supabase_realtime publication), so the in-app Notifications
-- screen + bell badge update instantly without refresh.
...
BEGIN
  ...
  ELSIF _target_type = 'agency' THEN
    ...
    SELECT owner_user_id INTO v_notify_uid FROM public.agencies WHERE id = _target_id;
  ...
  -- Pkg370: Instant in-app notification for the affected user.
...
-- Keep grants as set by Pkg365 (anon can call; internal admin verify is the gate).
...
'Pkg370: unified admin add/deduct for profile/helper/agency balances. Verifies admin session via x-admin-token header + section permission, bypasses protect_sensitive_profile_columns trigger for profile/agency writes, writes admin_logs, and emits an instant in-app notification to the affected user (delivered via Supabase Realtime).';