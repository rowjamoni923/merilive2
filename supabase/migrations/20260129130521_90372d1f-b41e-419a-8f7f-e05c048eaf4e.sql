...
-- Update the weekly transfer function to transfer ALL host beans completely
-- No beans should remain after transfer
  ...
  LOOP
    ...
    FOR v_host_record IN
      SELECT 
        ...
        COALESCE(p.beans, 0) as beans_balance
      FROM agency_hosts ah
      JOIN profiles p ON p.id = ah.host_id
      WHERE ah.agency_id = v_agency_record.agency_id
        AND ah.status = 'active'
        AND (COALESCE(p.total_earnings, 0) > 0 OR COALESCE(p.beans, 0) > 0)
    LOOP
      -- Calculate total host earnings (total_earnings + beans if applicable)
      ...
      IF v_host_earnings > 0 THEN
        ...
        -- Update agency beans balance (add commission)
        UPDATE agencies
        ...
        -- This ensures NO beans are left behind
        UPDATE profiles
        SET 
...
COMMENT ON FUNCTION public.process_weekly_agency_transfers() IS 
'Weekly automatic transfer: Takes ALL host earnings (total_earnings), 
calculates agency commission based on tier level, transfers commission to agency beans_balance, 
remaining goes to host pending_earnings, and resets host total_earnings to ZERO. 
No beans left behind after transfer.';
