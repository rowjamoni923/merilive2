...
-- Fix 1: Use exact COUNT(*) instead of pg_class estimate for total_users
-- Fix 2: Remove conflicting "No direct recharge inserts" policy that blocks Google Play inserts

-- Drop the blocking INSERT policy on recharge_transactions
DROP POLICY IF EXISTS "No direct recharge inserts" ON public.recharge_transactions;

-- Fix the dashboard stats RPC to use accurate count
