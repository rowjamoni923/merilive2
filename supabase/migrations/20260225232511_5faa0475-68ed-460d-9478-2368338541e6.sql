...
-- Drop and recreate the process_contact_violation function with safe UUID handling
...
BEGIN
    ...
    ELSIF v_penalty IS NOT NULL THEN
        ...
        UPDATE public.profiles
        SET beans = GREATEST(0, COALESCE(beans, 0) - v_beans_deducted)
        WHERE id = p_host_id;
    ELSE
    ...
    INSERT INTO public.chat_moderation_logs (
        ...
        END,
        true,
        'Auto-detected ' || p_detected_pattern || ' in ' || p_source_type || ' | Violation #' || v_new_violation_number || ' | -' || v_beans_deducted || ' beans'
    );
