...
-- The distribute_period_rewards function queries profiles table which has RLS.
-- Even SECURITY DEFINER may not bypass RLS on all internal queries via service_role context.
-- Fix: Add explicit SET row_level_security = off context, or use a simpler approach.

-- First, let's check and fix by recreating with explicit RLS bypass
...
-- Also fix auto_distribute to use SET role = postgres
CREATE OR REPLACE FUNCTION auto_distribute_leaderboard_rewards()
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
  ...
  v_cat TEXT;
  v_utc_hour INTEGER := EXTRACT(HOUR FROM v_now AT TIME ZONE 'UTC');
  v_utc_dow INTEGER := EXTRACT(DOW FROM v_now AT TIME ZONE 'UTC');
  v_utc_day INTEGER := EXTRACT(DAY FROM v_now AT TIME ZONE 'UTC');
BEGIN
  FOREACH v_cat IN ARRAY v_categories LOOP
    -- Daily: distribute every hour (idempotent - checks if already distributed)
    SELECT distribute_period_rewards(v_cat, 'daily') INTO v_count;
    IF v_count > 0 THEN
      v_results := v_results || v_cat || '/daily: ' || v_count || ' winners. ';
    END IF;

    -- Weekly: distribute on Monday (dow=1) - idempotent
    IF v_utc_dow = 1 THEN
    ...
    -- Monthly: distribute on 1st of month - idempotent
    IF v_utc_day = 1 THEN
  ...
  IF v_results = '' THEN
    v_results := 'No distributions needed (UTC hour: ' || v_utc_hour || ', DOW: ' || v_utc_dow || ', Day: ' || v_utc_day || ')';
  END IF;
