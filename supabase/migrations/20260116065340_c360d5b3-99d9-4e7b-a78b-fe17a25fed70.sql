...
-- Update the process_weekly_agency_transfers function to correctly update wallet_balance and clear host earnings
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      -- Use total_earnings directly (this is accumulated from gifts and calls)
      _total_host_earnings := COALESCE(_host.total_earnings, 0);
      
      -- Also get gift earnings from this period for records
      ...
      IF _total_host_earnings > 0 THEN
        ...
        -- Add to agency WALLET_BALANCE (this is what the dashboard shows)
        UPDATE agencies
        SET wallet_balance = COALESCE(wallet_balance, 0) + _agency_earnings,
            beans_balance = COALESCE(beans_balance, 0) + _agency_earnings
        WHERE id = _agency.id;
        
        -- CLEAR host earnings after transfer (this is the key fix!)
        UPDATE profiles
        SET total_earnings = 0,
            weekly_earnings = 0,
            pending_earnings = COALESCE(pending_earnings, 0) + _total_host_earnings
        WHERE id = _host.host_id;
        
        _total_transfers := _total_transfers + 1;
        _total_amount := _total_amount + _agency_earnings;
        
        RAISE NOTICE 'Transferred % to agency % for host % (Total Earnings: %)', 
          _agency_earnings, _agency.agency_name, _host.display_name, _total_host_earnings;
      END IF;
    END LOOP;
  END LOOP;
