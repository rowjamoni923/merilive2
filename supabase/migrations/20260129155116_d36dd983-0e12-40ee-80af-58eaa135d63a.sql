-- Fix the admin_process_withdrawal function to not update non-existent diamonds column
-- Instead, credit the helper's topup_helpers.wallet_balance (which is already done)
-- And also record in a separate field if needed
...
BEGIN
  ...
  ELSIF _action = 'reject' THEN
    -- Return money to agency beans_balance
    UPDATE agencies
  ...
  ELSIF _action = 'complete' THEN
    ...
    IF _withdrawal.assigned_helper_id IS NOT NULL THEN
      -- Use net_withdrawal_beans from payment_details as the diamond reward
      _net_beans := COALESCE(
      ...
      IF _diamond_reward > 0 THEN
        ...
        IF _helper_user_id IS NOT NULL THEN
          ...
          -- Credit helper's wallet with net reward (90%) in topup_helpers table
          -- This is the TRADER WALLET where helpers can sell diamonds
          ...
          -- Also credit the helper's total_earnings in profiles for tracking
          UPDATE profiles
          SET total_earnings = COALESCE(total_earnings, 0) + _net_reward::BIGINT
