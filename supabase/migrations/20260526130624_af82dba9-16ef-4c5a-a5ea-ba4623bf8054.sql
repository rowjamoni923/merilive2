...
BEGIN
  ...
  FROM eligible e
  WHERE
    -- Pkg368: hide offline hosts on every tab. Only show:
    --  (a) live now, (b) really online (heartbeat <30min & availability!=offline),
    --  (c) currently busy in a private call.
    (e.is_live_now OR e.is_really_online OR e.has_active_call)
    AND (
      (
