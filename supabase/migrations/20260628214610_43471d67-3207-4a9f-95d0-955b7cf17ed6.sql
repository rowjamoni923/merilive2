...
-- Patch service_auto_finalize_face_verification:
-- Move liveness_failed and profile_mismatch BELOW the strong_identity computation
-- and skip them when strong_identity holds. Hard rejects retained: duplicate_face
-- and account_gender_mismatch. Aligns with owner policy (2026-06-28):
-- "same person multiple photos must pass, only ~5% manual".
...
BEGIN
  ...
  -- Pull biometric scores BEFORE any soft-rejection so strong_identity can override.
  ...
  -- Strong identity = passive scan + complete evidence + same person + no
  -- identity mismatch + at least one biometric proof >= 75 (photo↔live OR
  -- face-video↔live OR profile-match OR host-gallery-min).
  v_strong_identity := v_passive_scan
    ...
    AND (
      v_photo_live >= 75
      OR v_face_video_live >= 75
      OR v_profile_match >= 75
      OR v_host_min >= 75
    );

  -- HARD rejects (kept regardless of strong identity).
  ...
  -- SOFT rejects: only block when strong identity does NOT hold.
  IF v_liveness_failed AND NOT v_strong_identity THEN
  ...
  END IF;
  IF v_profile_mismatch AND NOT v_strong_identity THEN
  ...
  ELSIF v_expected_gender IN ('male', 'female') AND v_final <> v_expected_gender THEN
    -- HARD: account gender vs detected gender stays a reject.
  ...
  SET status = 'approved',
      ...
      admin_notes = CASE
        WHEN trim(coalesce(admin_notes, '')) = '' THEN
          '[auto] service_auto_finalize_face_verification: approved (95% policy 2026-06-28 v2 - strong identity overrides liveness/profile mismatch).'
        ELSE
          trim(admin_notes) || E'\n[auto] service_auto_finalize_face_verification: approved (95% policy v2, strong_identity=' || v_strong_identity::text || ').'
      END,
      updated_at = now()
  ...
  RETURN jsonb_build_object(
    ...
    'liveness_failed_overridden', v_liveness_failed AND v_strong_identity,
    'profile_mismatch_overridden', v_profile_mismatch AND v_strong_identity,
...
-- Re-queue recent false rejects: rows rejected in the last 7 days where every
-- biometric signal is strong (>= 75) and there is no duplicate / gender / age /
-- replay issue. The face-verification-sweep cron + service_auto_finalize will
-- pick them up within ~1 minute and auto-approve under the new policy.
WITH candidates AS (
  SELECT s.id
  ...
  WHERE s.status = 'rejected'
    ...
    AND coalesce((s.ai_analysis -> 'rekognition' ->> 'identity_mismatch')::text, 'false') NOT IN ('true','t','1','yes')
    AND coalesce((s.ai_analysis -> 'rekognition' ->> 'evidence_same_person')::text, 'false') IN ('true','t','1','yes')
    AND coalesce((s.ai_analysis -> 'rekognition' ->> 'evidence_complete')::text, 'false') IN ('true','t','1','yes')
    AND (
      coalesce(nullif(s.ai_analysis -> 'rekognition' ->> 'photo_live_score','')::numeric, 0) >= 75
      OR coalesce(nullif(s.ai_analysis -> 'rekognition' ->> 'face_video_live_score','')::numeric, 0) >= 75
      OR coalesce(nullif(s.ai_analysis -> 'rekognition' ->> 'profile_match_score','')::numeric, 0) >= 75
      OR coalesce(nullif(s.ai_analysis -> 'rekognition' ->> 'host_photos_min_score','')::numeric, 0) >= 75
    )
    -- Guard the hard rejects we keep.
    AND NOT (
      lower(coalesce(s.ai_analysis -> 'rekognition' ->> 'final_gender','')) IN ('male','female')
      AND lower(coalesce(s.ai_analysis -> 'rekognition' ->> 'expected_gender','')) IN ('male','female')
      AND lower(s.ai_analysis -> 'rekognition' ->> 'final_gender') <>
          lower(s.ai_analysis -> 'rekognition' ->> 'expected_gender')
    )
)
...
SET status = 'pending',
    ...
    reviewed_at = NULL,
    reviewed_by = NULL,
    admin_notes = coalesce(trim(s.admin_notes), '') ||
      E'\n[re-queue 2026-06-28] reset to pending under 95% policy v2 (strong identity overrides liveness).',
    updated_at = now()
FROM candidates c
WHERE s.id = c.id;
