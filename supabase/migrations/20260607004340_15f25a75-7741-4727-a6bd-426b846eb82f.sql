...
BEGIN
  ...
  -- Bug #3 fix — write coin ledger debit row so SUM(coin_transactions.coins_amount)
  -- can be reconciled against profiles.coins. Skip silently if cost exceeds the
  -- INTEGER column capacity (defensive: column is INTEGER, not BIGINT).
