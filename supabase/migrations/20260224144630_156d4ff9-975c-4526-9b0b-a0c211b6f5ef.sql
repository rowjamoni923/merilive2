...
-- Fix: Create internal balance functions WITHOUT admin check for use by SECURITY DEFINER server-side functions
-- These are ONLY callable by other SECURITY DEFINER functions, not directly by users

-- Internal: add beans (no admin check)
...
-- Internal: add diamonds (no admin check)
...
-- Internal: add coins (no admin check)
...
-- Revoke direct execution from anon and authenticated roles (only callable internally)
...
-- Now update distribute_period_rewards to use internal functions
...
BEGIN
  ...
  IF p_category = 'game_winners' THEN
    ...
    FOR v_entry IN (
      ...
      IF v_reward IS NOT NULL THEN
        ...
        VALUES (v_entry.user_id, p_category, p_period_type, v_period_label, v_rank, v_entry.stat_value, COALESCE(v_reward.reward_coins,0), COALESCE(v_reward.reward_diamonds,0), COALESCE(v_reward.reward_beans,0));
        INSERT INTO notifications (user_id, type, title, message, data, is_read) VALUES (
          v_entry.user_id, 'reward', '🏆 Game Reward!',
          'You ranked #' || v_rank || ' in ' || CASE p_period_type WHEN 'daily' THEN 'Daily' WHEN 'weekly' THEN 'Weekly' ELSE 'Monthly' END || ' Game Rankings!',
          jsonb_build_object('category', p_category, 'period_type', p_period_type, 'rank', v_rank), false);
        v_count := v_count + 1;
      END IF;
    END LOOP;
  ...
  IF p_category = 'top_gifters' THEN
    ...
    FOR v_entry IN (
      ...
      IF v_reward IS NOT NULL THEN
        ...
        VALUES (v_entry.user_id, p_category, p_period_type, v_period_label, v_rank, v_entry.stat_value, COALESCE(v_reward.reward_coins,0), COALESCE(v_reward.reward_diamonds,0), COALESCE(v_reward.reward_beans,0));
        INSERT INTO notifications (user_id, type, title, message, data, is_read) VALUES (
          v_entry.user_id, 'reward', '🏆 Gifter Reward!',
  ...
  IF p_category = 'agency_performance' THEN
    v_rank := 0;
    FOR v_entry IN (
      SELECT ah.agency_id, SUM(COALESCE(p.total_earnings, 0)) AS stat_value
      FROM agency_hosts ah
      JOIN profiles p ON p.id = ah.host_id
      WHERE ah.status = 'active'
      GROUP BY ah.agency_id
      HAVING SUM(COALESCE(p.total_earnings, 0)) > 0
      ORDER BY stat_value DESC LIMIT 50
    ) LOOP
      v_rank := v_rank + 1;
      SELECT * INTO v_reward FROM leaderboard_reward_config WHERE category = p_category AND period_type = p_period_type AND is_active = true AND v_rank >= rank_from AND v_rank <= rank_to LIMIT 1;
