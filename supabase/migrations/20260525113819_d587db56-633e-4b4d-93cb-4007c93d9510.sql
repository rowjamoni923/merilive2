-- Pkg336: enforce host_availability server-side.

-- (1) Value whitelist via BEFORE INSERT/UPDATE trigger (avoids CHECK constraint
--     issues on existing rows + lets us coerce nulls to 'online').
CREATE OR REPLACE FUNCTION public.tg_guard_host_availability()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF NEW.host_availability IS NULL OR btrim(NEW.host_availability) = '' THEN
    NEW.host_availability := 'online';
  ELSE
    NEW.host_availability := lower(btrim(NEW.host_availability));
    IF NEW.host_availability NOT IN ('online','offline','busy') THEN
      RAISE EXCEPTION 'invalid host_availability value: %', NEW.host_availability
        USING ERRCODE = '22023';
    END IF;
  END IF;
  RETURN NEW;
...
DROP TRIGGER IF EXISTS guard_host_availability ON public.profiles;
CREATE TRIGGER guard_host_availability
BEFORE INSERT OR UPDATE OF host_availability ON public.profiles
FOR EACH ROW
EXECUTE FUNCTION public.tg_guard_host_availability();

-- (2) Block incoming calls when the host has toggled Go Offline.
--     Patch start_private_call to reject if receiver host_availability='offline'.
...
BEGIN
  ...
  -- Pkg336: respect host's Go Offline / Busy toggle.
