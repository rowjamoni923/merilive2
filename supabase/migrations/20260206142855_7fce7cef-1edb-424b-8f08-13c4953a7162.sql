...
-- Fix the distribute_period_rewards function - fix the FULL OUTER JOIN query
      ...
      IF v_reward IS NOT NULL THEN
        ...
        IF v_entry.owner_id IS NOT NULL THEN
          INSERT INTO notifications (user_id, type, title, message, data, is_read) VALUES (
            ...
            'Your agency ranked #' || v_rank || '! ' || COALESCE(v_reward.reward_diamonds, 0) || ' Diamonds credited.',
            jsonb_build_object('category', p_category, 'period_type', p_period_type, 'rank', v_rank,
              'reward_diamonds', COALESCE(v_reward.reward_diamonds, 0)), false);
        END IF;
