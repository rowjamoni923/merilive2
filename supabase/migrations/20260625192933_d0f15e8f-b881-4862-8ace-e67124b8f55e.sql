...
BEGIN
  ...
  IF v_passive_scan AND v_identity_mismatch THEN
    v_reject_reason := 'Verification rejected because the profile photo, verification video, and live face scan are not the same person. Please submit only your own real photo/video/live scan.';

    UPDATE public.face_verification_submissions
    SET status = 'rejected',
        rejection_reason = v_reject_reason,
        reviewed_at = coalesce(reviewed_at, now()),
        admin_notes = concat_ws(E'\n', nullif(trim(coalesce(admin_notes, '')), ''), format('[auto-reject] identity_mismatch DB guard: photo_live=%s face_video_live=%s intro_video_live=%s', coalesce(v_photo_live_score::text, 'null'), coalesce(v_face_video_live_score::text, 'null'), coalesce(v_intro_video_live_score::text, 'null'))),
        updated_at = now()
    WHERE id = p_submission_id;
    ...
    RETURN jsonb_build_object('success', false, 'reason', 'photo_video_live_identity_mismatch', 'auto_rejected', true, 'manual_review_requested', false, 'expected_gender', v_expected_gender, 'photo_live_score', v_photo_live_score, 'face_video_live_score', v_face_video_live_score, 'intro_video_live_score', v_intro_video_live_score);
  END IF;

  IF v_passive_scan AND (NOT v_evidence_complete OR NOT v_evidence_same_person) THEN
    RETURN jsonb_build_object(
      'success', false,
      'reason', CASE WHEN NOT v_evidence_complete THEN 'photo_video_live_evidence_missing' ELSE 'photo_video_live_identity_mismatch' END,
  ...
  SET status = 'approved',
      ...
      admin_notes = CASE
        WHEN trim(coalesce(admin_notes, '')) = '' THEN
          '[auto] service_auto_finalize_face_verification: photo + video + live scan same-person gate, duplicate/gender/liveness/Rekognition checks passed.'
        ELSE
          trim(admin_notes) || E'\n[auto] service_auto_finalize_face_verification: photo + video + live scan same-person gate, duplicate/gender/liveness/Rekognition checks passed.'
      END,
      updated_at = now()
