...
BEGIN
  ...
  -- Agency owners ALWAYS land in the agency diamond balance (= Trader Wallet).
  -- Helper-only (non-agency) users with level 1-5 / payroll land in the helper wallet.
  ...
  IF _destination = 'trader_wallet_helper' THEN
    ...
    -- helper_transactions columns: helper_id, transaction_type, amount,
    -- balance_before, balance_after, reference_id, description, user_id, created_at.
    -- There is NO `status` column — do not insert one.
