...
AS $$
  SELECT CASE
    WHEN lower(trim(coalesce(_status, ''))) = 'approved' THEN 'approved'
    WHEN lower(trim(coalesce(_status, ''))) = 'rejected' THEN 'rejected'
    ELSE 'pending'
  END;
$$;
...
REVOKE ALL ON FUNCTION public.face_verification_status_bucket(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.face_verification_status_bucket(text) TO authenticated, anon;
REVOKE ALL ON FUNCTION public.face_verification_is_auto_reviewed(text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.face_verification_is_auto_reviewed(text, text) TO authenticated, anon;
