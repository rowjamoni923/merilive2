...
BEGIN
  ...
  _account_name := btrim(COALESCE(_payment_details->>'account_name', ''));
  _account_number := btrim(COALESCE(_payment_details->>'account_number', ''));
  ...
  IF _account_name = '' OR length(_account_name) < 2 OR length(_account_name) > 80 OR _account_name !~ '^[A-Za-z][A-Za-z .''-]{1,79}$' THEN
