...
BEGIN
  ...
  SET is_host = false,
      host_status = CASE WHEN host_status IN ('blocked','rejected') THEN host_status ELSE 'pending_face' END,
      updated_at = now()
  WHERE is_host = true
    AND (face_verification_status IS DISTINCT FROM 'approved'
         OR COALESCE(is_face_verified,false) = false);

  UPDATE public.profiles
  SET is_face_verified = true, updated_at = now()
  WHERE face_verification_status = 'approved'
...
DROP TRIGGER IF EXISTS trg_enforce_host_face_invariant ON public.profiles;
CREATE TRIGGER trg_enforce_host_face_invariant
  BEFORE INSERT OR UPDATE ON public.profiles
  FOR EACH ROW EXECUTE FUNCTION public.enforce_host_face_invariant();

-- Patched approval RPC: face_verification_status set in same UPDATE
CREATE OR REPLACE FUNCTION public.admin_force_verify_and_approve_host(
  _user_id uuid, _approve_as text DEFAULT 'host',
  _set_gender text DEFAULT NULL, _reason text DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
DECLARE
  _caller_id uuid; _existing RECORD;
  _final_gender text; _final_role text;
  _face_url text; _safe_url text; _submission_id uuid;
BEGIN
  ...
  _final_gender := COALESCE(NULLIF(lower(trim(coalesce(_set_gender,''))),''),
                            CASE WHEN _final_role='host' THEN 'female' ELSE 'male' END);
  IF _final_gender NOT IN ('female','male') THEN
    _final_gender := CASE WHEN _final_role='host' THEN 'female' ELSE 'male' END;
  END IF;
...
-- Patched executor: face_verification_status set during approve/reject; is_host cleared on reject
