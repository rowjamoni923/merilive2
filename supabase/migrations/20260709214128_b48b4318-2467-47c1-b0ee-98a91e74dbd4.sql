...
BEGIN
  ...
  IF EXISTS (
    SELECT 1
     ...
     WHERE google_order_id = v_order_id
        OR transaction_id = v_order_id
  ) THEN
  ...
  UPDATE public.profiles
     SET coins = COALESCE(coins, 0) + v_credit_coins,
         total_recharged = COALESCE(total_recharged, 0) + v_credit_coins,
         updated_at = now()
   WHERE id = p_user_id
...
BEGIN
  ...
  UPDATE public.profiles
     SET coins = COALESCE(coins, 0) + v_pkg.total_coins,
         total_recharged = COALESCE(total_recharged, 0) + v_pkg.total_coins,
         updated_at = now()
   WHERE id = p_user_id
...
EXCEPTION WHEN unique_violation THEN
  ...
  RETURN jsonb_build_object('success', true, 'alreadyProcessed', true, 'coins', COALESCE(v_pkg.total_coins, 0), 'newBalance', COALESCE(v_balance_after, 0), 'transactionId', v_recharge_id, 'invitation', v_invite_result);
END;
$$;