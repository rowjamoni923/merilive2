...
BEGIN
  ...
  INSERT INTO public.user_beans_exchanges(
    user_id, beans_amount, diamonds_reward, exchange_rate, tier_id, status, completed_at, destination_type
  ) VALUES (
    _user_id, _beans_amount, _diamonds_reward, _exchange_rate, _tier_id, 'completed', now(), _destination
  );
...
BEGIN
  ...
  VALUES (_receiver_id, _amount, 'agency_transfer_in', 'completed', 'Agency transfer credited to user top-up balance from ' || COALESCE(v_agency_name, 'Agency'), _agency_id::text);
...
DECLARE
  ...
  v_target_helper_id uuid;
  v_new_target_wallet numeric;
BEGIN
  v_caller := auth.uid();
  ...
  SELECT id INTO v_target_helper_id
  FROM public.topup_helpers
  WHERE user_id = v_target_owner_id
    AND COALESCE(is_active, false) = true
    AND COALESCE(is_verified, false) = true
  LIMIT 1
  FOR UPDATE;

  IF v_target_helper_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Target agency owner does not have an active verified trader wallet');
  END IF;
  ...
  SET wallet_balance = COALESCE(wallet_balance, 0) + _amount,
      ...
      updated_at = now()
  WHERE id = v_target_helper_id
  RETURNING wallet_balance INTO v_new_target_wallet;

  INSERT INTO public.notifications (user_id, type, title, message, data, is_read, created_at)
  VALUES (
    v_target_owner_id,
    'trader_wallet_received',
    'Trader Wallet Received',
    _amount::text || ' coins received in trader wallet from ' || COALESCE(v_sender_agency_name, 'Agency'),
    jsonb_build_object('from_agency_id', _sender_agency_id, 'from_agency_name', v_sender_agency_name, 'target_agency_id', _target_agency_id, 'amount', _amount, 'balance_bucket', 'trader_wallet', 'action_url', '/helper-dashboard'),
    false,
    now()
  );
  ...
  RETURN jsonb_build_object(
    ...
    'new_target_wallet_balance', v_new_target_wallet,
    'target_agency_id', _target_agency_id,
    'target_helper_id', v_target_helper_id,
...
REVOKE EXECUTE ON FUNCTION public.agency_send_diamonds_to_agency(uuid, uuid, integer) FROM PUBLIC, anon;
