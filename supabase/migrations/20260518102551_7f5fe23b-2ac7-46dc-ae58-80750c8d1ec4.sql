-- Harden face verification approval so old profile triggers cannot reset approved rows back to pending.
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF coalesce(NEW.is_face_verified, false) THEN
      NEW.face_verification_status := 'approved';
    ELSE
      NEW.face_verification_status := coalesce(nullif(trim(NEW.face_verification_status), ''), 'pending');
    END IF;
    RETURN NEW;
  END IF;
  ...
  ELSIF NEW.face_verification_status = 'rejected' THEN
    NEW.face_verification_status := 'rejected';
...
BEGIN
  ...
  ELSIF _action_type='process_face_verification' THEN
    ...
    IF v_action='approve' THEN
      ...
      -- Final authoritative write happens AFTER profile triggers have run so the list can never remain pending.
      UPDATE public.face_verification_submissions
         SET status='approved', verification_type=v_approve_as,
             reviewed_by=public.current_admin_id_from_header(), reviewed_at=COALESCE(reviewed_at, now()),
...
-- Repair existing mismatches in both directions.
