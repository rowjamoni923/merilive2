...
-- 1. Owner-session helper: must be callable by anon/authenticated so RLS policies referencing it don't raise "permission denied" when evaluated for non-owners. The function itself only returns true for verified owners.
GRANT EXECUTE ON FUNCTION public.is_active_admin_owner_session() TO anon, authenticated, PUBLIC;

-- 2. Relax profile-protection trigger: still locks money / host / identity / security, but lets the row owner update their own login-audit fields and backfill registration audit columns when NULL.
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    -- Money / earnings / levels / subscriptions (ALWAYS locked)
    ...
    -- Host / verification / moderation state (ALWAYS locked)
    ...
    -- Agency / identity / auth-contact / security (ALWAYS locked)
    ...
    -- Registration audit: allow first-time backfill (NULL -> value) by row owner; otherwise locked.
    ...
    -- Login audit (last_login_*): always writable by row owner; otherwise locked.
    ...
    -- Moderation counters and system-controlled profile assets (ALWAYS locked)
...
-- 3. Missing UNIQUE for `topup_helpers.user_id` (AdminHelperApplications / AdminHelperManagement upsert).
ALTER TABLE public.topup_helpers
  ADD CONSTRAINT topup_helpers_user_id_key UNIQUE (user_id);

-- 4. Missing UNIQUE for leaderboard_podium_frames (rank_position, category) (AdminLeaderboardManagement upsert).
CREATE UNIQUE INDEX IF NOT EXISTS leaderboard_podium_frames_rank_category_key
  ON public.leaderboard_podium_frames (rank_position, category);
