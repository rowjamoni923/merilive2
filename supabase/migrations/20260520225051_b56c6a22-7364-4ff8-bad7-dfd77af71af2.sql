...
BEGIN
  ...
  IF v_face_count IS DISTINCT FROM 1 THEN
    RETURN jsonb_build_object('success', false, 'reason', 'invalid_face_count', 'face_count', v_face_count, 'manual_review_requested', v_manual_flag);
  END IF;
  IF v_left_count IS DISTINCT FROM 1 OR v_right_count IS DISTINCT FROM 1 THEN
    RETURN jsonb_build_object('success', false, 'reason', 'invalid_side_face_count', 'left_face_count', v_left_count, 'right_face_count', v_right_count, 'manual_review_requested', v_manual_flag);
  END IF;
  ...
  IF v_front_err <> '' OR v_left_err <> '' OR v_right_err <> '' THEN
    RETURN jsonb_build_object('success', false, 'reason', 'angle_error', 'front_error', nullif(v_front_err,''), 'left_error', nullif(v_left_err,''), 'right_error', nullif(v_right_err,''), 'manual_review_requested', v_manual_flag);
  END IF;
  ...
  IF v_final NOT IN ('male', 'female') THEN
    RETURN jsonb_build_object('success', false, 'reason', 'invalid_final_gender', 'manual_review_requested', v_manual_flag);
  END IF;

  IF v_gender_conflict THEN
    RETURN jsonb_build_object('success', false, 'reason', 'gender_conflict', 'manual_review_requested', v_manual_flag);
  END IF;
  ...
  IF v_front_yaw IS NULL OR v_left_yaw IS NULL OR v_right_yaw IS NULL THEN
    RETURN jsonb_build_object('success', false, 'reason', 'missing_pose_yaw', 'manual_review_requested', v_manual_flag);
  END IF;

  v_left_delta := abs(v_left_yaw - v_front_yaw);
  v_right_delta := abs(v_right_yaw - v_front_yaw);
  v_lr_delta := abs(v_left_yaw - v_right_yaw);
  IF v_left_delta < 7 OR v_right_delta < 7 OR v_lr_delta < 12 THEN
    RETURN jsonb_build_object(
      'success', false,
      'reason', 'insufficient_side_pose',
      'front_pose_yaw', v_front_yaw,
      'left_pose_yaw', v_left_yaw,
      'right_pose_yaw', v_right_yaw,
      'manual_review_requested', v_manual_flag
    );
  END IF;

  IF v_age_high < 18 THEN
    RETURN jsonb_build_object('success', false, 'reason', 'underage', 'manual_review_requested', v_manual_flag);
  END IF;

  IF v_occ > 88 THEN
    RETURN jsonb_build_object('success', false, 'reason', 'face_occluded', 'manual_review_requested', v_manual_flag);
  END IF;
