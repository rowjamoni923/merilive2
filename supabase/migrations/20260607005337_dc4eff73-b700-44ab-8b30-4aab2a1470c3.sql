...
WITH dups AS (
  SELECT id,
         ROW_NUMBER() OVER (PARTITION BY user_id, recharge_id ORDER BY applied_at ASC, id ASC) AS rn
    FROM public.vip_recharge_bonus_log
   WHERE recharge_id IS NOT NULL
)
DELETE FROM public.vip_recharge_bonus_log v
 USING dups
 WHERE v.id = dups.id AND dups.rn > 1;

CREATE UNIQUE INDEX IF NOT EXISTS uniq_vip_recharge_bonus_per_recharge
  ON public.vip_recharge_bonus_log (user_id, recharge_id)
  WHERE recharge_id IS NOT NULL;
