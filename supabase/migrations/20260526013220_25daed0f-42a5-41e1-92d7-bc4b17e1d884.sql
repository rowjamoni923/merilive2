...
BEGIN
  ...
  -- Hard fraud signals → manual review (never auto-approve)
  ...
  -- Account-type vs detected gender MUST match (hard fraud check).
  ...
  -- Pro-app confidence floors: 60% face/gender catches blurry/junk submissions
  -- but accepts normal selfies. (Was 70% — rejected too many real users.)
  IF v_gender_conf < 60 OR v_face_conf < 60 THEN
  ...
  -- Was 50% on either side — caused false rejects when one side photo was poor.
  ...
  -- REMOVED: insufficient_side_pose check. The 5°/10° yaw-delta gate was the
  -- single biggest false-reject source — many real users hold the phone steady
  -- and only turn slightly. The replay_suspected check above (<3° all three)
  -- still catches static-photo spoofs, and the side-similarity check above
  -- still catches "different person on side photos" attacks.
