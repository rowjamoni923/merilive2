-- Fix process_weekly_agency_transfers: Skip Level 5 Payroll Helper agencies during reset
-- They ALWAYS stay at A5/12%, never reset to A1/3%
...
BEGIN
  ...
  LOOP
    ...
    IF v_is_payroll_helper THEN
      -- Payroll helpers NEVER reset — always A5/12%
      UPDATE agencies
      SET level = 'A5',
          commission_rate = 12,
          updated_at = now()
    ...
    ELSE
      -- Regular agencies reset to A1/3% for new week
      UPDATE agencies
      SET level = 'A1',
          commission_rate = 3,
          updated_at = now()
  ...
  v_result := jsonb_build_object(
    ...
    'agencies_reset', true,
    'timestamp', now(),
    'message', format('Processed %s hosts. Host earnings: %s beans → agency wallet. Company commission: %s beans → agency wallet. Payroll helpers kept at A5/12%%, others reset to A1/3%%.', 
