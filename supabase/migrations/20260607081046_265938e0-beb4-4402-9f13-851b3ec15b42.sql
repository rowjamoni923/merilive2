...
BEGIN
  IF COALESCE(OLD.is_active, false) = true
    ...
    UPDATE public.stream_viewers
       SET left_at = COALESCE(left_at, NEW.ended_at, now())
     WHERE stream_id = NEW.id
       AND left_at IS NULL;

    IF NEW.viewer_count IS DISTINCT FROM 0
       OR NEW.status IS DISTINCT FROM 'ended'
       OR NEW.ended_at IS NULL
       OR COALESCE(NEW.is_active, false) IS DISTINCT FROM false THEN
      UPDATE public.live_streams
         SET viewer_count = 0,
             status = 'ended',
             ended_at = COALESCE(ended_at, NEW.ended_at, now()),
             is_active = false
       WHERE id = NEW.id;
    END IF;
...
AFTER UPDATE OF is_active, ended_at, status ON public.live_streams
FOR EACH ROW
EXECUTE FUNCTION public.tg_live_streams_close_viewers_on_end();

CREATE OR REPLACE FUNCTION public.close_live_stream_now(p_stream_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  SELECT id, host_id, is_active, ended_at
  INTO v_stream
  ...
  IF v_stream.host_id IS DISTINCT FROM uid THEN
  ...
  RETURN jsonb_build_object('success', true, 'stream_id', p_stream_id, 'ended_at', now());
END;
$$;

REVOKE ALL ON FUNCTION public.close_live_stream_now(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.close_live_stream_now(uuid) TO authenticated, service_role;
...
UPDATE public.live_streams
 ...
 WHERE id IN (
   ...
   WHERE COALESCE(is_active, false) = true
     ...
     AND COALESCE(last_heartbeat, started_at, created_at) < now() - interval '2 minutes'
 );