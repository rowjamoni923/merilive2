...
DECLARE
  ...
  _min_rate numeric := 0;
  _max_rate numeric := 0;
  _min_custom_level integer := 6;
  _custom_is_level_rate boolean := false;
BEGIN
  ...
  BEGIN
    ...
    _min_rate := COALESCE(NULLIF((_settings->>'min_rate')::numeric, 0), 0);
    _max_rate := COALESCE(NULLIF((_settings->>'max_rate')::numeric, 0), 0);
    _min_custom_level := COALESCE((_settings->>'min_level_for_custom_rate')::integer, 6);
  ...
  IF _receiver_avail = 'offline' THEN
    RETURN jsonb_build_object('success', false, 'error', 'host_offline', 'message', 'This host is currently offline.');
  ELSIF _receiver_avail = 'busy' THEN
    RETURN jsonb_build_object('success', false, 'error', 'host_busy', 'message', 'This host is currently busy.');
  END IF;
  ...
  SELECT COALESCE(host_level, 0), call_rate_per_minute
    INTO _host_level, _host_custom_rate
    FROM public.profiles WHERE id = p_receiver_id;

  IF COALESCE(_host_custom_rate, 0) > 0 THEN
    IF _level_rates IS NOT NULL AND jsonb_typeof(_level_rates) = 'array' THEN
      SELECT EXISTS (
        SELECT 1 FROM jsonb_array_elements(_level_rates) lr
        WHERE (lr->>'rate')::numeric = _host_custom_rate
      ) INTO _custom_is_level_rate;
    END IF;

    IF _custom_is_level_rate
       OR (
         COALESCE(_host_level, 0) >= _min_custom_level
         AND (_min_rate <= 0 OR _host_custom_rate >= _min_rate)
         AND (_max_rate <= 0 OR _host_custom_rate <= _max_rate)
       ) THEN
  ...
  IF EXISTS (SELECT 1 FROM public.private_calls WHERE status = 'connected' AND (caller_id = p_caller_id OR host_id = p_caller_id)) THEN
...
REVOKE ALL ON FUNCTION public.start_private_call(uuid, uuid, text, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.start_private_call(uuid, uuid, text, uuid) FROM anon;
