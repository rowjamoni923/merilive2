...
BEGIN
  ...
  ELSIF _is_agency_owner = true THEN
  ...
  IF _tier_id IS NOT NULL THEN
    ...
    _settings_key := 'tier';
  ELSE
    -- Pick the correct settings bucket so the server's expected diamond
    -- amount matches what the client (which reads the same precedence)
    -- computed at submit time. Agency owners use the agency-specific
    -- bucket (rate + fee policy can differ); everyone else uses the
    -- shared bucket. Fall back gracefully when one of the two is missing.
