-- Normalize text settings that were accidentally stored as JSON-quoted strings.
  ...
  SET status = 'approved',
      ...
      admin_notes = CASE
        WHEN trim(coalesce(admin_notes, '')) = '' THEN
          '[auto] service_auto_finalize_face_verification: account type matched live face and strict liveness/Rekognition thresholds passed.'
        ELSE
          trim(admin_notes) || E'\n[auto] service_auto_finalize_face_verification: account type matched live face and strict liveness/Rekognition thresholds passed.'
      END,
      updated_at = now()
