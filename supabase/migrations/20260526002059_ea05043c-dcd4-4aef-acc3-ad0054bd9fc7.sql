...
-- Pkg344: Withdrawal end-to-end fixes
-- (1) Persist top-level columns so admin queries + guards see country/currency/usd
-- (2) Dedupe helper notification triggers (one path, both helper_notifications + notifications)
-- (3) Dedupe owner status notification triggers

-- (1) Rewrite request_agency_withdrawal to populate top-level columns
...
BEGIN
  ...
  -- Extract & sanitize top-level columns from payment_details
  ...
  v_pm_type := CASE
    WHEN v_method IN ('epay','crypto_auto','usdt','usdttrc20') THEN 'auto'
    ELSE 'manual'
  END;
...
REVOKE EXECUTE ON FUNCTION public.request_agency_withdrawal(uuid, numeric, text, jsonb, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.request_agency_withdrawal(uuid, numeric, text, jsonb, text) TO authenticated;

-- (2) Drop duplicate helper notification trigger (kept the more complete one that writes BOTH helper_notifications and notifications)
DROP TRIGGER IF EXISTS trg_notify_payroll_helpers_on_agency_withdrawal ON public.agency_withdrawals;

-- (3) Drop duplicate owner status notification trigger (the surviving trigger has better Approved/Rejected wording)
DROP TRIGGER IF EXISTS trigger_notify_withdrawal_status ON public.agency_withdrawals;

-- (4) Backfill top-level country_code from payment_details for legacy rows that still have NULL
UPDATE public.agency_withdrawals aw
SET country_code = upper(NULLIF(regexp_replace(aw.payment_details->>'country_code','[^A-Za-z]','','g'),''))
WHERE aw.country_code IS NULL
  AND aw.payment_details ? 'country_code';

UPDATE public.agency_withdrawals aw
SET currency = upper(NULLIF(regexp_replace(aw.payment_details->>'currency_code','[^A-Za-z]','','g'),''))
WHERE aw.currency IS NULL
  AND aw.payment_details ? 'currency_code';
