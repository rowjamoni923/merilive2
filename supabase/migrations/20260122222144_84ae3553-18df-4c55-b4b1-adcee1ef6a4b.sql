-- Update process_weekly_agency_transfers to also reset call_rate_per_minute to 0
-- This will trigger the frontend to auto-select Level 1 rate when host_level is 0
  ...
  LOOP
    ...
    LOOP
      ...
      IF _total_host_beans > 0 THEN
        ...
        -- CLEAR ALL host earnings including beans and reset call rate to Level 1
        -- host_level will be calculated dynamically from weekly_earnings (which is now 0)
        -- So host_level effectively becomes 0, and call_rate resets to Level 1 rate
        UPDATE profiles
        ...
        RAISE NOTICE 'Transferred % beans from host % to agency %. Call rate reset to Level 1 (%)', 
          _total_host_beans, _host.display_name, _agency.agency_name, _level1_rate;
      END IF;
    END LOOP;
  END LOOP;
