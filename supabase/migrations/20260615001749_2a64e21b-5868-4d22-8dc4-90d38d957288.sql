...
BEGIN
  ...
  SELECT id, COALESCE(coins, 0)::bigint AS coins,
    ...
    INTO _sender FROM public.profiles WHERE id = p_sender_id;
  ...
  IF _total_cost <= 2147483000 THEN
    INSERT INTO public.coin_transactions (
      ...
      p_sender_id, (-_total_cost)::integer, 'gift_send', 'completed',
      'gift_tx:' || _transaction_id::text || ' gift:' || p_gift_id::text || ' qty:' || _qty::text,
      now(), now()
    );
  END IF;
  -- LUCKY GIFT LOTTERY — Chamet-style ladder w/ MEGA JACKPOT tail. RTP ≈ 79%.
  IF _is_lucky THEN
    SELECT EXISTS (
    ...
    FOR _i IN 1.._qty LOOP
      ...
      IF _has_cfg THEN
        ...
        END LOOP;
      ELSE
        -- Default Chamet-style ladder. EV ≈ 0.791 (house edge ≈ 21%).
        -- Tiers (cumulative): 55 / 85 / 95 / 98 / 99.5 / 99.9 / 99.98 / 99.998 / 99.9998 / 100.
        IF _roll < 55.0 THEN
          _unit_bonus := 0;                                              -- 55% no win
        ELSIF _roll < 85.0 THEN
          _unit_bonus := FLOOR(_gift.coin_value * 0.5)::bigint;          -- 30% 0.5x
        ELSIF _roll < 95.0 THEN
          _unit_bonus := FLOOR(_gift.coin_value * 1)::bigint;            -- 10% 1x
        ELSIF _roll < 98.0 THEN
          _unit_bonus := FLOOR(_gift.coin_value * 2)::bigint;            -- 3% 2x
        ELSIF _roll < 99.5 THEN
          _unit_bonus := FLOOR(_gift.coin_value * 5)::bigint;            -- 1.5% 5x
        ELSIF _roll < 99.9 THEN
          _unit_bonus := FLOOR(_gift.coin_value * 20)::bigint;           -- 0.4% 20x
        ELSIF _roll < 99.98 THEN
          _unit_bonus := FLOOR(_gift.coin_value * 100)::bigint;          -- 0.08% 100x
        ELSIF _roll < 99.998 THEN
          _unit_bonus := FLOOR(_gift.coin_value * 500)::bigint;          -- 0.018% 500x EPIC
        ELSIF _roll < 99.9998 THEN
          _unit_bonus := FLOOR(_gift.coin_value * 2000)::bigint;         -- 0.0018% 2000x MEGA
        ELSE
          _unit_bonus := FLOOR(_gift.coin_value * 10000)::bigint;        -- 0.0002% 10000x LEGEND
        END IF;
      END IF;
    ...
    IF _diamond_bonus > 0 THEN
      ...
      UPDATE public.profiles SET coins = _new_sender_balance WHERE id = p_sender_id;
      IF _diamond_bonus <= 2147483000 THEN
        INSERT INTO public.coin_transactions (
          user_id, coins_amount, transaction_type, status, notes, created_at, updated_at
        ) VALUES (
          p_sender_id, _diamond_bonus::integer, 'lucky_bonus', 'completed',
