...
BEGIN
  ...
  -- Step 1: Try helper wallet first
  SELECT *
  INTO helper_rec
  ...
  -- Step 2: Try agency diamond_balance
  IF remaining > 0 THEN
    SELECT *
    INTO agency_rec
    ...
    ELSIF agency_rec IS NOT NULL AND COALESCE(agency_rec.diamond_balance, 0) > 0 THEN
      ...
      UPDATE public.agencies
      SET diamond_balance = 0, updated_at = now()
      WHERE id = agency_rec.id;
    END IF;
  END IF;

  -- Step 3: If still insufficient, rollback partial deductions
  IF remaining > 0 THEN
    IF helper_deducted > 0 AND helper_rec IS NOT NULL THEN
      UPDATE public.topup_helpers
      SET wallet_balance = COALESCE(wallet_balance, 0) + helper_deducted, updated_at = now()
      WHERE id = helper_rec.id;
    END IF;
    IF agency_deducted > 0 AND agency_rec IS NOT NULL THEN
      UPDATE public.agencies
      SET diamond_balance = COALESCE(diamond_balance, 0) + agency_deducted, updated_at = now()
      WHERE id = agency_rec.id;
    END IF;
    ...
    RETURN jsonb_build_object(
      ...
      'debug_helper_found', helper_rec IS NOT NULL,
      'debug_helper_wallet', COALESCE(helper_rec.wallet_balance, 0),
      'debug_agency_found', agency_rec IS NOT NULL,
      'debug_agency_balance', COALESCE(agency_rec.diamond_balance, 0),
      'debug_remaining', remaining,
      'debug_amount_requested', _amount,
  ...
  -- Step 4: Credit user's diamond balance
