...
-- Update the process_weekly_agency_transfers function to transfer ALL beans and clear everything
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      -- Calculate TOTAL beans from ALL sources (total_earnings + pending_earnings + weekly_earnings)
      _total_host_beans := COALESCE(_host.total_earnings, 0) + 
                          COALESCE(_host.pending_earnings, 0) + 
      ...
      IF _total_host_beans > 0 THEN
        ...
        INSERT INTO agency_earnings_transfers (
          ...
          'Full beans transferred: ' || _total_host_beans || ', Commission: ' || _agency_earnings
        );
        
        -- Add FULL beans to agency wallet_balance
        UPDATE agencies
        SET wallet_balance = COALESCE(wallet_balance, 0) + _total_host_beans,
            beans_balance = COALESCE(beans_balance, 0) + _total_host_beans
        WHERE id = _agency.id;
        
        -- CLEAR ALL host earnings - NO beans remain in host account
        UPDATE profiles
        ...
        RAISE NOTICE 'Transferred % beans from host % to agency % (Commission: %)', 
          _total_host_beans, _host.display_name, _agency.agency_name, _agency_earnings;
      END IF;
    END LOOP;
  END LOOP;
