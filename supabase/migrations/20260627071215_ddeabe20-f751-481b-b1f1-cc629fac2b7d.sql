...
BEGIN
  ...
  -- Sanitize host_photos: only keep the non-empty URLs the host actually
  -- uploaded in Step-2 gallery. These get copied to the public profile ONLY
  -- on approval — never before. Unapproved/rejected accounts never publish
  -- evidence photos. NULL means "don't overwrite what's already there".
  IF v_sub.host_photos IS NOT NULL THEN
    SELECT array_agg(u) INTO v_host_photos
    FROM (
      SELECT unnest(v_sub.host_photos) AS u
    ) s
  ...
  SET is_host = (v_expected_gender = 'female'),
      ...
      host_photos = CASE
        WHEN v_expected_gender = 'female' AND v_host_photos IS NOT NULL AND array_length(v_host_photos, 1) > 0
          THEN v_host_photos
        ELSE host_photos
      END,
  ...
  SET status = 'approved',
      ...
      admin_notes = CASE
        WHEN trim(coalesce(admin_notes, '')) = '' THEN
          '[auto] service_auto_finalize_face_verification: approved — Step-1 photo set as profile picture, Step-2 host photos published to profile gallery.'
        ELSE
          trim(admin_notes) || E'\n[auto] service_auto_finalize_face_verification: approved — Step-1 photo set as profile picture, Step-2 host photos published to profile gallery.'
      END,
      updated_at = now()
