...
DROP FUNCTION IF EXISTS public.can_initiate_private_call(uuid, uuid, uuid);
DROP FUNCTION IF EXISTS public.start_private_call(uuid, uuid, text);
DROP FUNCTION IF EXISTS public.start_private_call(uuid, uuid, text, uuid);
...
BEGIN
  ...
  IF p_context_stream_id IS NULL THEN
    IF EXISTS (SELECT 1 FROM public.live_streams ls WHERE ls.host_id = p_receiver_id AND ls.ended_at IS NULL AND COALESCE(ls.is_active,true) = true AND lower(COALESCE(ls.status,'active')) = 'active') THEN
