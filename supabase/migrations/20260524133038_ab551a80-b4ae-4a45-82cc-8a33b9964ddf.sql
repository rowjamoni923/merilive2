-- ============================================================
-- Pkg307 pass-3: Private call deep hardening
-- ============================================================
...
CREATE OR REPLACE FUNCTION public.submit_private_call_rating(
  p_call_id uuid,
  p_rating integer,
...
BEGIN
  ...
  IF v_uid <> v_call.caller_id AND v_uid <> v_call.host_id THEN
    RAISE EXCEPTION 'not authorized to rate this call' USING ERRCODE = '42501';
  END IF;

  IF v_call.status <> 'ended' THEN
    RETURN jsonb_build_object('success', false, 'error', 'call_not_ended');
  END IF;

  IF v_uid = v_call.caller_id THEN
    UPDATE public.private_calls
       SET host_rating = p_rating,
           updated_at = now()
     WHERE id = p_call_id;
  ELSE
    UPDATE public.private_calls
       SET caller_rating = p_rating,
           updated_at = now()
     WHERE id = p_call_id;
  END IF;
  ...
  VALUES (
    ...
    jsonb_build_object(
      'rating', p_rating,
      'review', v_review,
      'rated_by', CASE WHEN v_uid = v_call.caller_id THEN 'caller' ELSE 'host' END,
      'rated_user_id', CASE WHEN v_uid = v_call.caller_id THEN v_call.host_id ELSE v_call.caller_id END
    )
  );
...
BEGIN
  ...
  IF v_call.status = 'ended' AND v_call.ended_at IS NOT NULL THEN
...
BEGIN
  ...
  IF _call IS NULL OR _call.host_id IS NULL OR _call.host_id <> auth.uid() THEN
    RAISE EXCEPTION 'Invalid call or not authorized' USING ERRCODE = '42501';
  END IF;
...
BEGIN
  ...
  PERFORM set_config('app.bypass_profile_protection', 'true', true);
  PERFORM 1 FROM public.profiles WHERE id IN (p_caller_id, p_receiver_id) FOR UPDATE;
  ...
  IF EXISTS (SELECT 1 FROM public.private_calls WHERE status IN ('pending', 'ringing', 'connected') AND (caller_id = p_caller_id OR host_id = p_caller_id)) THEN
    RETURN jsonb_build_object('success', false, 'error', 'caller_busy_in_call');
  END IF;
...
REVOKE EXECUTE ON FUNCTION public.submit_private_call_rating(uuid, integer, text) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.settle_private_call(uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.finalize_first_minute_earnings(uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.start_private_call(uuid, uuid, text, uuid) FROM PUBLIC, anon;
REVOKE EXECUTE ON FUNCTION public.accept_private_call(uuid) FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.submit_private_call_rating(uuid, integer, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.settle_private_call(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.finalize_first_minute_earnings(uuid) TO authenticated;
