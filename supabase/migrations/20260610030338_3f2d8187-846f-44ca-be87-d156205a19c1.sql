...
BEGIN
  ...
  -- Unified rule: ALL beans-to-diamond exchanges land in the Helper (Trader) Wallet
  -- whenever the user is an active L1-L5 helper trader (or payroll-enabled).
  -- Agency owners without a helper account fall back to spendable coins.
  ...
  IF _destination = 'trader_wallet_helper' THEN
    ...
    INSERT INTO public.helper_transactions (
      helper_id, transaction_type, amount, reference_id, description, user_id, status, created_at
    ) VALUES (
      _helper_id, 'beans_exchange_credit', _diamonds_reward, _user_id,
      'Beans exchanged to trader (helper) wallet', _user_id, 'completed', now()
    );
  ELSE
