...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    -- PKG-FACE-NOTIF-FIX 2026-06-27: include 'user' + NULL verification_type
    -- (non-host face verifications). Only 'host' rows route to host_applications.
    'face_verifications_pending', (
      SELECT count(*) FROM public.face_verification_submissions
      WHERE status IN ('pending', 'submitted', 'under_review')
        AND (verification_type IS NULL OR verification_type IN ('face', 'user', 'profile', 'selfie'))
    ),
