-- Section 12 — agency host self-service leave + weekly helpers
...
AS $$
  SELECT COALESCE(
    (SELECT weekly_earnings::bigint FROM public.profiles WHERE id = COALESCE(_uid, auth.uid()) LIMIT 1),
    0::bigint
  );
$$;
  ...
  SELECT COALESCE(
    (
      ...
      WHERE ap.agency_id = _agency_id
        ...
        AND ap.period_start = date_trunc('week', (timezone('utc', now()))::date)::date
