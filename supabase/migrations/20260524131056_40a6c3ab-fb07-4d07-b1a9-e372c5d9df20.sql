...
-- ============================================================
-- Pkg307: Private call IDOR + DoS hardening
-- ============================================================

-- 1) timeout_private_call: require auth + must be call participant
...
-- 2) decline_private_call: remove anon bypass (was: IS NOT NULL)
...
-- 3) end_private_call: remove anon bypass (was: IS NOT NULL)
...
-- 4) finalize_first_minute_earnings: must be the call's host
...
BEGIN
  ...
  IF auth.uid() <> v_call.caller_id AND auth.uid() <> v_call.host_id THEN
...
-- 5) start_private_call: DO NOT kill receiver's connected calls
--    (Previously the receivers_stale sweep could end any 'connected' row
--     older than 2s, letting any caller forcibly disconnect a host.)
...
BEGIN
  ...
  -- DEAD-FOREVER for CALLER's own stale rows (caller is auth-verified above).
  ...
  -- RECEIVER stale-sweep: ONLY orphan pending/ringing (never connected).
  -- A 'connected' call belongs to the receiver and a third party. Killing it
  -- would let any caller forcibly disconnect an in-progress call (DoS / kick).
  ...
  -- General expired sweep — pending/ringing rows past timeout
  WITH expired AS (
    UPDATE public.private_calls
       SET status = 'missed', ended_at = now(), end_reason = 'timeout', updated_at = now()
     WHERE status IN ('pending', 'ringing')
  ...
  -- Clear stuck busy-flag pointing at no-longer-active rows
  UPDATE public.profiles p
     SET is_in_call = false, current_call_id = NULL, updated_at = now()
...
-- 6) Lock down execute privs — authenticated only
...
REVOKE EXECUTE ON FUNCTION public.deduct_call_coins_per_minute(uuid) FROM PUBLIC, anon;
...
REVOKE EXECUTE ON FUNCTION public.mark_call_delivered(uuid,text,jsonb) FROM PUBLIC, anon;
...
GRANT EXECUTE ON FUNCTION public.deduct_call_coins_per_minute(uuid) TO authenticated;
