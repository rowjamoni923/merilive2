...
// Owner policy (2026-07-03): face-verification thresholds are NO LONGER
// admin-tunable. Values are hardcoded here to industry-pro defaults
// (Chamet/Bigo-class). `app_settings` overrides are intentionally ignored so
// the admin panel cannot accidentally loosen fraud safety. Change these
// constants in code + redeploy if the policy ever needs to shift.
const SAME_PERSON_MIN_SIMILARITY = 55;
const DUPLICATE_FACE_MIN_SIMILARITY = 85;
const PROVIDER_DUPLICATE_SEARCH_THRESHOLD = 80;

// Kept as a no-op so callers/tests don't break. Intentionally does nothing —
// admin panel is not the source of truth for these values anymore.
async function loadFaceThresholdsFromAdmin(_supabaseAdmin: any): Promise<void> {
  return;
}
