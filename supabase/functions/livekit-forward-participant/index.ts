// Pkg128 — LiveKit Move/Forward Participant
//
// Moves a participant from one LiveKit room to another using
// `RoomServiceClient.forwardParticipant(srcRoom, identity, dstRoom)`.
// The participant disconnects from src and auto-reconnects to dst with
// the same identity; no client-side rejoin code required.
//
...
async function validateAdminToken(
  ...
  try {
    ...
    console.warn("[livekit-forward] admin validate failed:", e);
    return { ok: false };
  }
}
...
async function resolveHostOwnership(
  ...
  try {
    ...
    console.warn("[livekit-forward] host resolve failed", e);
    return null;
  }
}
...
async function killSwitchOn(admin: ReturnType<typeof createClient>): Promise<boolean> {
  try {
    ...
    return v?.forward_participant === true;
  } catch {
    return false;
  }
}
...
Deno.serve(async (req) => {
  ...
  if (!(await killSwitchOn(auditClient))) {
    return json(403, { error: "forward_participant_disabled" });
  }
  ...
  if (adminToken) {
    ...
    // Hosts may forward only from rooms they own.
  ...
  const audit = async (success: boolean, errorMessage?: string) => {
    try {
      await auditClient.from("livekit_participant_forwards").insert({
      ...
      console.warn("[livekit-forward] audit insert failed", e);
    }
  };

  try {
    // SDK 2.9.x signature: forwardParticipant(srcRoom, identity, dstRoom).
    // Falls back to TwirpRPC if the SDK method shape differs.
    ...
    if (typeof anyRs.forwardParticipant !== "function") {
      throw new Error("forwardParticipant_not_supported_by_sdk");
    }
    const result = await anyRs.forwardParticipant(srcRoom, identity, dstRoom);
    await audit(true);
    return json(200, { success: true, result: result ?? { forwarded: identity } });
  } catch (e) {
    const msg = String((e as Error)?.message ?? e);
    console.error("[livekit-forward] failed", msg);
    await audit(false, msg);
    return json(500, { error: "forward_failed", message: msg });
  }
});
