...
- [ ] **N3e — Native → window CustomEvent bridge.** Update `livekitConnectionQuality.ts` / `livekitActiveSpeaker.ts` / `livekitMetadata.ts` / `livekitRoomMetadata.ts` / `livekitTranscription.ts` so each module's `register(scope, id, room)` also accepts a native session and dispatches the same `livekit-*` window CustomEvents from native plugin events. Wraps N3b plumbing.
