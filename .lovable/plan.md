...
- Fix: `/auth` no longer deletes device recovery keys; “profile missing → re-register” fallback screens no longer delete `meri_device_id`; `persistentDeviceId` now restores old encrypted recovery keys and backs up the ID to native secure storage; if a matching device account is found but session exchange fails, registration now fail-closes instead of creating a duplicate account.
