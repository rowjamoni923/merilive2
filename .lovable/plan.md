...
- [x] **N3a — H.264 codec lock + bitrate ceiling.** DONE 2026-06-08. `resolvePublishCodec()` (LiveKitPlugin.kt:4054) now defaults `auto` → `h264` when HW encoder available (else VP9 → AV1 → SDK default). Industry standard (Bigo/Chamet) — 30-40% lower host CPU vs VP8. Existing `setPreferredCodec` JS API still wins when user pins one.
- [x] **N3b — Plugin event gaps.** DONE 2026-06-08. Added 5 native event emissions in `attachEventListeners` (LiveKitPlugin.kt:1996-2054): `active-speakers-changed`, `participant-metadata-changed`, `participant-name-changed`, `room-metadata-changed`, `transcription-received`. Field names verified against livekit-android 2.23.4 bytecode. JS-side window-event bridge to existing `livekitActiveSpeaker.ts` / `livekitMetadata.ts` / `livekitRoomMetadata.ts` / `livekitTranscription.ts` deferred to N3e (needs scope/id plumbing from caller hooks).
- [x] **N3c — Native subscriber video quality cap.** DONE 2026-06-08. Added `setSubscriberVideoQuality({sid?, quality, source?})` + `setRemoteVideoSubscribed({sid?, subscribed, source?})` `@PluginMethod`s in `LiveKitPlugin.kt`. Verified `RemoteTrackPublication.setVideoQuality(VideoQuality.LOW|MEDIUM|HIGH)` + `setSubscribed(bool)` against livekit-android 2.23.4 bytecode. TS API mirrors them in `NativeLiveKit.ts`. `livekitAudioOnlyMode.ts.applyAudioOnlyToRoom` now ALSO calls the native plugin when available — JS path retained for parity. Industry pattern (Agora `setRemoteVideoStreamType` / Bigo `setSubscribeMode`).
- [x] **N3d — Token refresh native bridge.** DONE 2026-06-08. Researched livekit-android 2.23.4: no public `updateAuthToken` API exists (SDK has only server-pushed `RTCEngine.onRefreshToken` callback). Industry pattern (Bigo/Agora `renewToken` = also requires reconnect). Implementation: new `refreshToken({token})` `@PluginMethod` updates `lastConnectArgs.token` so the next hard-reconnect (resilience watchdog, network drop, server kick) carries the fresh JWT. `livekitTokenRefresh.ts.applyTokenToRoom` now ALSO pushes to native plugin when available. Test = let token approach expiry, observe silent rotation via resilience reconnect path.
- [x] **N3e — Native → window CustomEvent bridge.** DONE 2026-06-08. Extended `useNativeLiveKitEvents(active, handlers, bridge?)` with optional `bridge: {scope, id}`. When provided, listens to native `active-speakers-changed` / `participant-metadata-changed` / `room-metadata-changed` / `transcription-received` and re-dispatches as the SAME `livekit-active-speakers` / `livekit-participant-metadata` / `livekit-room-metadata` / `livekit-transcription` window CustomEvents with matching detail shapes. Wired in `useLiveKitClient` (scope=`live`, id=`liveSignalingStreamId`), `useLiveKitCall` (scope=`call`, id=`callId`), `usePartyRoomNativeLiveKit` (scope=`party`, id=`roomId`). All four existing React consumer hooks (useActiveSpeakers / useParticipantMetadata / useRoomMetadata / useTranscription) now work transparently on native sessions — zero consumer-side change.
- [x] **N3f — RPC + text-stream bridges.** DONE 2026-06-08. Researched livekit-android 2.23.4: RPC stable since 2.12.0 (`RpcManager.registerRpcMethod` + `LocalParticipant.performRpc`, String payloads, `RpcError` exceptions, `responseTimeout` clamped ≥8s). Data Streams stable since 2.14.0 (`sendText(text, StreamTextOptions)` returns `Result<TextStreamInfo>`; `registerTextStreamHandler(topic, handler)` delivers `TextStreamReceiver` with `Flow<String>` chunks). Equivalent to Agora RTM/Bigo signaling for in-room request/response (PK invite, gift ack, moderation, raise-hand). Added 6 `@PluginMethod`s in `LiveKitPlugin.kt`: `registerRpcMethod`, `unregisterRpcMethod`, `respondToRpc`, `performRpc`, `sendText`, `registerTextStreamHandler`, `unregisterTextStreamHandler`. RPC handler is async — native suspends SDK handler on a `CompletableDeferred` until JS resolves via `respondToRpc(requestId, result|errorMessage)`; SDK timeout clamped 250ms under transport limit for friendlier error surface. Text-stream handler collects `Flow<String>` and emits `text-stream-chunk` + `text-stream-complete` events. Added TS surface in `NativeLiveKit.ts` (6 methods + 3 listener events: `rpc-invocation`, `text-stream-chunk`, `text-stream-complete`) and standalone opt-in helper `src/lib/livekitNativeMessaging.ts` (`tryRegisterNativeRpcMethod` / `tryPerformNativeRpc` / `tryRegister(Unregister)NativeTextStreamHandler` / `trySendNativeText`) honoring `isLiveKitEnabled('rpc'|'chat')` kill-switches. Existing `livekitRpc.ts` / `livekitChatAttachments.ts` consumers NOT rewired to keep risk near zero — opt-in only.
- [x] **N3g — Camera release hardening.** VERIFIED 2026-06-08 — already complete. `disconnect()` (LiveKitPlugin.kt:1279) already does: `setCameraEnabled(false)` + `setMicrophoneEnabled(false)` + `BeautyPipelineBridge.setEnabled(false)` + `OEM_CAMERA_RELEASE_SETTLE_MS` delay + `room.disconnect()` + `releaseRoomResources` (calls `room.release()`) + `CameraOwnership.release(OWNER_LIVEKIT)`. Matches research §7e pattern (CameraXHelper provider is process-static; SDK has no public `unregisterCameraProvider` API — lifecycle is owned by Activity).
- [x] **N3h — Test updates.** DONE 2026-06-08. Refactored stale UnifiedPartyRoom seat-cleanup assertion (component moved from inline `videoRef`/timer pattern to `LiveKitVideoPlayer` + memoized track adapter — invariants `el.srcObject = null` on detach + authoritative host-position lookup still locked). Added 2 new describe blocks (`native LiveKit bridge — Kotlin plugin events` + `native LiveKit bridge — TS surface`) with 11 assertions covering N3b event emissions, N3c quality/subscribe PluginMethods, N3d refreshToken, N3e scope/id bridge wiring across all 3 React hooks (useLiveKitClient/useLiveKitCall/usePartyRoomNativeLiveKit), N3f RPC + text-stream methods/events + `livekitNativeMessaging.ts` helper + kill-switch honor, N3g disconnect() ordering (camera off → settle → release → ownership). 28/28 tests passing.
- [x] **N3i — Owner APK test prep.** DONE 2026-06-08. Test plan written to `.lovable/n3i-owner-apk-test.md` — 6 sections (prereqs, host live, viewer live, private call, party room, stress/regression) + optional devtools event verification. Each box maps to specific N3a-N3h invariants (camera-state event, OEM release ordering, setSubscriberVideoQuality, refreshToken-on-reconnect, window CustomEvent fanout for active-speakers/metadata/transcription). Owner runs after APK rebuild on physical device + second peer.
