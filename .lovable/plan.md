...
- Root cause in app: `FaceVerification.tsx` native CameraX mode intentionally made the WebView transparent and the preview effectively full-screen, while submit navigated away without immediately tearing down the camera or locking the UI to `submitted`. Fix applied: native camera is visually masked to the scan card, submit now stops native/web camera immediately, locks duplicate submits, switches to the pending screen, and invokes `face-verification-analyze` immediately as a client-side backup while the DB trigger/sweeper remains authoritative.
