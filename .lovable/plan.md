...
- [x] R2-H10 raw-image purge (Wave-2, 2026-06-10): new `purge-face-verification-images` edge fn (cron-only, gated by `CRON_SECRET`) deletes every storage object under the private `face-verification` bucket belonging to submissions with status in (`approved`, `rejected`, `expired`) older than 7 days, then NULLs the `*_url` columns and stamps `images_purged_at`. Pages 200 rows at a time, retries failed deletes on the next run (only the timestamp is written when storage delete succeeds). EDPB 2024 + Onfido/Persona TOS compliant. Bucket was already private + service-role-only so 15-min signed-URL TTL is not needed today — left as a follow-up if we ever expose direct URLs to authenticated clients.
- [x] R2-H11 reels upload guard (Wave-2): tightened `storage.objects` INSERT policy `Authenticated users can upload reels` to additionally require `metadata->>'mimetype'` starts with `video/` AND lowercase extension ∈ {mp4, mov, webm, m4v}. Closes MIME-spoof / unknown-container abuse on the reels bucket while keeping the existing per-user folder rule. Face bucket is service-role-write only (clients upload via signed POST through the verification edge fn) so no policy change needed there.
- [x] Log scrubbing batch (Wave-2): `verify-google-purchase` no longer logs the full Google Play response (was leaking obfuscated externalAccountId + developerPayload) — only the 5 safe state fields. `swift-pay-create-deposit` no longer logs the raw gateway body on errors (was leaking provider txn ids + addresses) — status + sanitized message only.
...
**Bucket A — Private Call hardening (H-4, H-11, H-13) — ✅ DONE 2026-06-10**
- H-4 (callEndedRef consolidation): `usePrivateCall` now exports its internal `callEndedRef`; `CallProvider` consumes the SAME ref instead of maintaining a parallel one. Eliminates the drift class where the parent ref stayed `true` after the hook reset (stuck "ended" gate that blocked the next incoming call) or stayed `false` after the hook ended (phantom active-call screen). Single source of truth for every read site (`handleEndCall`, `handleAcceptCall`, `shouldShowActiveCall`, debug log, "Track host status" effect).
- H-11 (StrictMode channel safety): new `privateCallChannelRef` in `usePrivateCall` holds the active `private-call-${userId}` Realtime channel. Effect now tears down any prior subscription BEFORE opening a new one and clears the ref only when its own channel is being torn down — prevents two channels sharing the same name on React 18 StrictMode dev double-mount or on `userId` flip (account switch). No duplicate event fan-out, no zombie listener after sign-out.
