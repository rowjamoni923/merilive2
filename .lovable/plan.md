...
**Research applied:** LiveKit KB says this exact error should be diagnosed as WebRTC media connectivity (WebSocket/WebRTC/TURN checks), and LiveKit firewall docs split media connection into ICE candidate probing then DTLS/SRTP; mobile networks may need TURN relay fallback. Competitor-standard live apps (Chamet/Bigo/Agora category) keep room UI responsive and retry/fallback network transport rather than leaving hosts on an endless camera loader.
