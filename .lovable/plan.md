...
- Immediate fixes applied: chat pending video preview now uses real local object URL video preview, chat media signed URLs last 7 days per session, chat video bubbles autoplay muted/inline instead of generic icon, face-verification uploaded/recorded intro video previews autoplay real frames, private face-verification signing fallback now stores a resolvable storage reference instead of a broken public URL, face-verification analyze/rerun edge functions now understand raw private storage references, and live feed/card fallback now uses real host/avatar imagery instead of `/placeholder.svg`.
