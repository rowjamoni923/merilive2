        ...
        <div>
          ...
          <p className="text-slate-600 text-sm">
            {verificationRecording ? `Step ${currentInstruction + 1} of ${faceInstructions.length}` : 'AI-powered identity verification'}
                  ...
                  {(() => {
                    const stepId = (instructions[currentInstruction]?.id ?? 'center') as string;
                    ...
                    // Lighting: no face for several ticks usually = light/exposure
                    const lighting: 'ok' | 'warn' | 'error' = !liveDiag.faceDetected
                      ? (consecutiveFailsRef.current >= 3 ? 'error' : 'warn')
                      : 'ok';

                    // Distance / framing: face missing OR way off baseline → likely too far / off-frame
                    const distance: 'ok' | 'warn' | 'error' = !liveDiag.faceDetected
                      ? 'warn'
                      : (ady > 35 || adp > 35) ? 'warn' : 'ok';

                    // Alignment (centering): only relevant on the center step OR
                    // when the engine is asking the user to face the camera straight.
                    const alignmentMatters = stepId === 'center' || hint.includes('face the camera') || hint.includes('level your head');
                    ...
                    // Head angle for the active turn/tilt step
                    ...
                    // Only flag items the engine actually cares about right now;
                    // collapse pure-OK rows visually so the list draws attention
