...
const Auth = () => {
  ...
  // 🚀 SESSION CHECK: Only redirect if user already has an active session
  // Device recovery is ONLY triggered by Start button — never automatically
  useEffect(() => {
    const checkExistingSession = async () => {
      ...
      try {
        // Check if user already has an active session FIRST
        ...
        // 🔒 On native: try restoring session from Capacitor Preferences
        // WebView localStorage may be cleared by OS on app kill
        ...
        // NO auto-recovery here — user must press Start button
  ...
  const handleStartClick = async () => {
    ...
    try {
      ...
      if (existingAccount) {
        ...
        console.log('[Auth] Attempting to restore account for:', existingAccount.displayName);
        
        // Attempt 1: Direct login with deterministic credentials
        ...
        // Attempt 2: Convert anonymous account to guest credentials, then retry
        console.log('[Auth] Login failed, attempting anonymous→guest conversion...');
        try {
          ...
          if (!convertError && convertResult?.converted) {
            console.log('[Auth] Conversion success, retrying login...');
          ...
          console.error('[Auth] Conversion failed:', conversionErr);
        }
        
        // Attempt 3: Account exists but all login methods failed
        // DO NOT create a new account — show error to user
        toast({
          title: "Account Recovery Issue",
          description: `An account (${existingAccount.displayName}) already exists on this device. Please try again or use Email login.`,
          variant: "destructive",
        });
        setLoading(false);
        return; // ← STOP HERE, never create duplicate
      }
      
      // SECOND: Check localStorage for saved credentials (backup recovery)
      if (deviceAccount?.email && deviceAccount?.password) {
        ...
        // Login failed - clear stale data
        localStorage.removeItem("meri_device_account");
        setDeviceAccount(null);
      }
      
      // THIRD: Truly no account for this device — show gender+name form
