...
import { ArrowLeft, Package, Clock, Check, X, Search, Eye, MessageCircle, Filter, RefreshCw, Globe } from "lucide-react";
    ...
    <div className="admin-pro-shell">
      ...
      <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 p-6 rounded-b-3xl shadow-lg">
        <div className="flex items-center gap-3 mb-4">
          <Button variant="ghost" size="icon" className="text-slate-900 hover:bg-white/20" onClick={() => navigate('/admin/diamond-traders')}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex items-center gap-2 text-sm text-slate-500">
                        ...
                        <Avatar className="w-5 h-5">
                          <UserAvatarImage seed={(((order.helper as any)?.user)?.id ?? ((order.helper as any)?.user)?.user_id ?? ((order.helper as any)?.user)?.host_id)} gender={((order.helper as any)?.user)?.gender} src={(order.helper as any)?.user?.avatar_url} />
