      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          ...
          >
            ...
            <span className="text-xs font-semibold uppercase tracking-[0.15em] text-blue-700">
              Agency & Payroll
            </span>
          </motion.span>
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="relative rounded-3xl overflow-hidden mb-14 border border-slate-200 shadow-2xl shadow-blue-500/5"
        >
          <img loading="lazy" decoding="async" src={agencyHeroImg} alt="Agency Team" className="w-full h-72 md:h-80 object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-white via-white/60 to-transparent" />
          <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12">
            <h3 className="text-3xl md:text-4xl font-extrabold mb-3 text-slate-900">
              Agency <span className="text-blue-600">Commission Tiers</span>
            </h3>
          ...
          {commissionTiers.map((tier, i) => (
            ...
            >
              ...
              <div className="relative">
                <div className="text-xl font-extrabold bg-gradient-to-r from-blue-600 to-purple-700 bg-clip-text text-transparent mb-2">
                  {tier.level}
                </div>
                <div className="text-3xl font-black text-slate-900 mb-1">{tier.rate}</div>
                <div className="text-[10px] text-slate-400 font-medium">{tier.income}</div>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        >
          <h3 className="text-2xl md:text-3xl font-extrabold text-center mb-3 text-slate-900">
            Automated <span className="bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">Payroll System</span>
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div className="rounded-2xl overflow-hidden border border-blue-500/10 shadow-xl shadow-blue-500/5">
              <img loading="lazy" decoding="async" src={payrollImg} alt="Payroll System" className="w-full h-72 object-cover" />
            </div>
            <div className="flex flex-col justify-center space-y-5">
              {[
                ...
                <motion.div
                  ...
                  className={`flex gap-4 p-4 rounded-xl bg-gradient-to-r ${item.bg} border border-slate-200 hover:border-slate-200 transition-all`}
                >
              ...
              {[
                ...
                <div key={step.label} className="flex items-center gap-3 md:gap-6">
                  <div className="flex flex-col items-center">
                    ...
                    <span className="text-[10px] text-slate-500 font-medium max-w-[80px]">{step.label}</span>
                  </div>
                  {i < 4 && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-14">
          {[
            { title: "Training & Mentorship", desc: "Get guidance from experienced agency owners to boost your streaming skills and audience.", icon: TrendingUp, gradient: "from-blue-500/15 to-cyan-500/10", border: "border-blue-500/15", iconColor: "text-blue-600" },
            { title: "Higher Visibility", desc: "Agencies promote their hosts through events, competitions, and in-app features.", icon: Users, gradient: "from-purple-500/15 to-indigo-500/10", border: "border-purple-500/15", iconColor: "text-purple-600" },
            { title: "Sub-Agency Bonus", desc: "Parent agencies earn 2% bonus from sub-agency hosts — build your network and grow!", icon: DollarSign, gradient: "from-emerald-500/15 to-green-500/10", border: "border-emerald-500/15", iconColor: "text-emerald-600" },
          ].map((item, i) => (
            <motion.div
              key={item.title}
              initial={{ opacity: 0, y: 30 }}
            ...
            >
              ...
              <div className="relative">
                <div className="w-14 h-14 mx-auto mb-5 rounded-2xl bg-slate-50 flex items-center justify-center group-hover:bg-slate-100 transition-colors">
        ...
        <motion.div
          ...
          className="mb-14"
        >
          <h3 className="text-2xl font-extrabold text-center mb-2 text-slate-900">
            Active <span className="text-blue-600">Agencies</span>
          </h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {agencies.map((agency, i) => (
              <motion.a
                ...
                className="flex items-center gap-3 p-4 rounded-xl bg-white shadow-sm border border-slate-200 hover:border-blue-500/30 hover:bg-white transition-all duration-300 group cursor-pointer"
              >
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-semibold text-slate-800 truncate">{agency.name}</h4>
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] text-slate-500 font-mono">{agency.agency_code}</span>
                    {agency.total_hosts !== null && agency.total_hosts > 0 && (
                      <span className="text-[10px] text-blue-600/70">· {agency.total_hosts} hosts</span>
                    )}
                  </div>
                </div>
                <button
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleCopy(agency.agency_code); }}
        ...
        >
          ...
          <p className="text-xs text-slate-400 mt-4">Or join an existing agency by copying their code above</p>
        </motion.div>
      </div>
    </section>
  );
