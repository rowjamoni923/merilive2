...
// Shooting Star decoration — PR-2.4: randoms frozen via useMemo.
...
export function UnifiedPartyRoom({
  ...
  const handleSendMessage = async (message: string) => {
    ...
    // Host sender + detected violation → server-side deduction + warning
    if (hostViolationDetected) {
      detectAndProcessViolation(sendingUserId, trimmedMessage, 'chat', sendingRoomId, false)
        .then(res => {
      ...
      <div ref={bottomControlsRef} className="absolute bottom-kb left-0 right-0 z-[90] pb-[calc(env(safe-area-inset-bottom)+0.75rem)] chat-composer-stable">
        <div className="px-2 flex items-center gap-1.5 bg-gradient-to-t from-black/70 via-black/30 to-transparent pt-2 pb-3">
          ...
          <motion.button
            ...
            className={cn(
              "w-9 h-9 rounded-full flex items-center justify-center shadow-lg shrink-0",
              isMuted 
          ...
          <motion.button
            ...
            className={cn(
              "w-9 h-9 rounded-full text-white flex items-center justify-center shadow-lg shrink-0",
