...
const maybeUpgradeToWidestCamera = async (
  ...
  try {
    ...
    for (const video of wideConstraints) {
      try {
        ...
        releaseAndroidWebViewCameraWhenStopped(wideStream, `wide-camera:${facingMode}`);
        console.log('[Camera] Upgraded to widest available camera:', best.label || best.deviceId);
        return wideStream;
      } catch (error: any) {
        console.warn('[Camera] Wide camera candidate failed:', error?.name, error?.message);
      }
    }
  } catch (error) {
    console.warn('[Camera] Wide camera selection skipped:', error);
  }
