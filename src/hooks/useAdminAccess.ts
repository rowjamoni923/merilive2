...
const withAdminAccessTimeout = async <T,>(promise: Promise<T>, label: string): Promise<T> => {
...
export const useAdminAccess = () => {
  ...
  const { data: verifiedAdminId, isLoading: isLoadingVerifiedAdmin } = useQuery({
    ...
    queryFn: async () => {
      ...
      let { data, error } = await withAdminAccessTimeout(
      ...
      if ((error || !data) && session?.device_fingerprint) {
        try {
          await withAdminAccessTimeout(adminSupabase.rpc('admin_request_device_access' as any, {
            _admin_id: adminId,
          ...
          const retry = await withAdminAccessTimeout(
            adminSupabase.rpc('current_admin_id_from_header' as any),
