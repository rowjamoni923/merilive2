  ...
  try {
    room.remoteParticipants.forEach((p: RemoteParticipant) => {
      p.trackPublications.forEach((pub: RemoteTrackPublication) => {
        // Keep camera video subscribed permanently. Older behavior unsubscribed
        // remote video for data saver, which made visitors see a blank/hidden
        // host camera. Data saver now only lowers quality; it never hides video.
        // @ts-ignore — Track.Kind shape varies across livekit-client minor versions
        if (pub.kind !== "video") return;
        try {
          pub.setSubscribed(true);
          pub.setVideoQuality?.(enabled ? VideoQuality.LOW : VideoQuality.HIGH);
        } catch {
          // ignore
        }
      });
