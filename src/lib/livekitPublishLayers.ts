 ...
 *
 * 📱 PROFESSIONAL PORTRAIT CAMERA: every preset is 9:16 vertical (height > width),
 * matching phone-screen previews so live/party/call do not look horizontal.
 *
...
export interface PublishLayerConfig {
  ...
  label: string;
  description: string;
  /** Capture resolution sent to camera (professional portrait 9:16). */
...
export const PUBLISH_LAYER_PRESETS: Record<PublishLayerTier, PublishLayerConfig> = {
  low: {
    ...
    description: "Single 540x960 portrait layer — best for weak uplink / 3G.",
  ...
  medium: {
    tier: "medium",
    label: "Medium",
    description: "720x1280 portrait base + 540x960 relay — balanced.",
  ...
  high: {
    ...
    description: "720p portrait base + 540x960 relay — full simulcast, default.",
    resolution: { width: 720, height: 1280, frameRate: 30 },
  ...
  ultra: {
    tier: "ultra",
    label: "Ultra (premium)",
    description: "1080p portrait base + 720p + 540p — fastest uplink only.",
