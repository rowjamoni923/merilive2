...
class AdminFramesScreen extends StatefulWidget {
  const AdminFramesScreen({super.key});

  @override
  State<AdminFramesScreen> createState() => _AdminFramesScreenState();
}
...
class _AdminFramesScreenState extends State<AdminFramesScreen> {
  ...
  Future<void> _loadFrames() async {
    ...
    try {
      final res = await _api.getSupabase().from('avatar_frames').select().order('created_at', ascending: false);
      if (mounted) {
        setState(() {
            ...
            children: [
              ...
              FadeInDown(
                child: Column(
                  ...
                  children: [
                    Text("AVATAR FRAMES HUB", style: GoogleFonts.outfit(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w900)),
                    const Text("Manage premium profile frames, noble status borders, and event specific decorations", style: TextStyle(color: Colors.white24, fontSize: 13)),
                  ],
                ),
              ),
            ],
        ...
        return FadeInUp(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.network(_api.resolveAssetUrl(frame['image_url'], bucket: 'avatar_frames'), width: 56, height: 56, errorBuilder: (c,e,s) => const Icon(LucideIcons.user, color: Colors.white10, size: 40))),
                const SizedBox(height: 16),
                Text(frame['name'] ?? 'Frame Name', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 11), maxLines: 1, overflow: TextOverflow.ellipsis),
                const SizedBox(height: 16),
                Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
